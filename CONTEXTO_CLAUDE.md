...
**Última actualización: 2026-06-04 (CIERRE jornada, 4ª regeneración). Sesión enorme: C2.2 OCR DE RECEPCIÓN CERRADO ENTERO Y EN PRODUCCIÓN (a-1 lee+valida, a-2 materializa, b.1 casa con memoria, b.2 create-on-scan, b.3 aprende, b.4 intermediario, b.5 anti-duplicado, b.6 copiloto de alta IA). ~10 commits, build verde, 0 0. La sesión previa (03/06) cerró el pedido rediseñado + MULTI-LOCAL + nº pedido + PDF + MOTOR DE IVA versionado.**
...
1. **MÓDULO SUPPLY — C2 RECEPCIÓN + C2.2 OCR: COMPLETOS Y EN PRODUCCIÓN (04/06).** Ya NO son frente. FRENTES NUEVOS de esta sesión (con disparador):
   - **C2.2.c — AVISOS COPILOTO en recepción** (siguiente natural): salto de precio vs last_price ("este artículo subió 18% respecto a la última compra"), avisos de caducidad. Era el siguiente sub-tramo de b.
   - **MULTI-ROL recepción + CAPTURA IN-SITU** (idea Julio 04/06): recibir desde el PORTAL DEL TRABAJADOR (no solo manager/oficina) → borrador; oficina valida → confirma. Y la CÁMARA integrada en el flujo de recepción (foto en el momento, no antes). Configurable por cuenta. Reutiliza campana manager APPCC + comunicación multicanal Fase A. Benchmark R365/MarketMan antes de diseñar.
...
7. Seguridad: rotar service_role/webhook tokens (Last `247ef137-...`). Code-splitting (~692KB gzip). 34 platos needs_review. Medidor coste IA por cuenta.
...
- **Webhooks externos** deploy `--no-verify-jwt`. **database.ts:** `gen types --yes` + UTF-8 sin BOM tras cada cambio de esquema.
