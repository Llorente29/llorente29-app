...
> **ESTADO 09/06:** FRENTE 1 y FRENTE 2 de abajo (fiabilidad canónico + motor de consumo teórico) están **HECHOS y verificados** — ya NO son frente. El frente activo es **RECEPCIÓN USABLE Y FIABLE** (ver §1.3, ítem 0). Lo de abajo se conserva como referencia del plan previo.

1. **FRENTE 1 — FIABILIDAD reorientada a canónico (EN CURSO):** capa 4 (señal de fiabilidad: RPC % ciego por importe/periodo, lee `unmapped_reason` del canónico — la consumen food cost/inventario/compras); reescribir `compute_sale_line_cost` para leer canónico (no raw_products); jubilar identidad del recast viejo. Luego capas 5-7 (impacto stock, alarmas, avisos).
2. **FRENTE 2 — Motor de consumo teórico** (desbloqueado al casar fiable): sale_line canónica × recipe_line → stock_movement consumo. Habilita AvT real. Contempla modificadores/combos (ya en canónico).
3. **FRENTE 3 — Limpieza de catálogo** (611 ingredientes muertos, proveedores duplicados).
4. **Sidebar "Modificadores por revisar"** (repaso global G3, cuando haya volumen). **Poblar escandallos base** (cascarón sin receta).
5. **Migración Llorente29** (poblar cuenta real). **Pulido demo** (responsive, DNS, perfil).
6. **Producción Llorente29 objetivo: 7 sept 2026.**

> Frentes previos ya resueltos (E1 ficha producto, Supply C2/C3, inventario capa 1) siguen pendientes de PROBAR EN VIVO pero no son frente de construcción.
