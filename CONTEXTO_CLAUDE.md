...
>
> **(G2e COMPLETO + DASHBOARD "FOLVY SHOP → INICIO" — CERRADO Y VERIFICADO, commits 6234560→eeb7f88, migraciones T2670..T2700).** Panel por campaña (campaign_performance: canjes vivos, invertido, ventas atribuidas, ticket medio CON vs SIN la campaña en el mismo periodo, margen real, ROI verde/ámbar/rojo, serie diaria, honestidad "N canjes sin margen"). **G2e.2 — deuda del MOTOR saldada:** los canjes de las ofertas de item grababan margin_after NULL pese a existir los datos → ahora margen real por atribución a nivel de orden (item_percent/bogo = v_subtotal−v_cost_known; free_delivery resta además el envío; free_item resta el coste real del regalo, null-propaga sin escandallo); merge anclado sobre place_shop_order post-2640, **regresión de cobro verificada**; la verificación del valor queda latente al PRIMER CANJE real (lunes). G2e.3 = pestaña Rendimiento en Campañas (hero KPIs, donut "¿qué oferta te funciona?", top ROI clicable al panel, vacío comercial). **G2e.4 = el dashboard de mando "Folvy Shop → Inicio"** (petición de Julio: "algo comercial, vendible a un hostelero"; benchmark Otter Dashboard/Analytics — cada métrica vs periodo anterior, filtros potentes, promoted-vs-unpromoted, ROI instantáneo; goleada: MARGEN REAL donde Otter da ventas brutas): **maqueta-primero en 2 rondas** (lección del Hub; v2 con las 3 correcciones de Julio: dropdowns multi-select con buscador+chips, gráficas frescas Linear/Stripe sobre recharts custom, Descargar ▾ con 5 informes CSV/XLSX patrón Otter) → cableado con `shop_home_overview` todo-en-una y comparación por VENTANA ESPEJO (Δ del hero = requisito), el módulo ABRE en Inicio, informes reales → **verificado en vivo con datos reales**. `KpiCard` extraído como componente embebible. **PRINCIPIO NUEVO (Julio): dashboard POR MÓDULO + Inicio de la app como MOSAICO CONFIGURABLE por cliente** — frente futuro declarado; todo KPI nace embebible.
>
> **(G2d MOTOR DE REGLAS — FRENTE ACTIVO al cierre; diseño v1 APROBADO, doc docs/folvy_g2d_motor_reglas_v1.md, Code construyendo por sub-lotes: modelo+auditoría → evaluador con valle horario → los otros 2 disparadores → UI Reglas).** La campaña se enciende SOLA — el terreno de Pleez con la verdad que Pleez no tiene (histórico PROPIO por SELECT, no scraping; margen real por disparo). Modelo: `campaign_rule` (trigger_type + condition jsonb EXTENSIBLE, action_template, scope, **budget_max OBLIGATORIO**, cooldown_minutes, max_active, duration) + `campaign_rule_firing` (auditoría: por qué se encendió, snapshot de la condición). Evaluador `evaluate_campaign_rules()` con pg_cron `*/15` (patrón del job 7 expire-unpaid-shop-orders, confirmado en RECON). 3 disparadores v1 contra histórico propio: **valle horario** (franja actual vs media de esa franja+día en N semanas, dropPct), **marca floja** (7d vs media móvil vía sale_line→brand), **plato parado con stock alto** (current_stock alto + ventas bajas). Campañas de regla nacen `origin='rule'` (columna de G1), badge "Automática (regla)", pausables/eliminables por humano = kill switch. LÍMITES: presupuesto obligatorio + cooldown + max_active por regla + **tope global 3/cuenta**. **VISIBILIDAD obligatoria (adición de Julio/Claude): el disparo se VE sin buscarlo** — banner/contador en Campañas + "disparó hace 2h → campaña X" en la fila de la regla; notificación admin declarada para cuando exista el canal. Clima (Open-Meteo) y eventos = v2, costura lista (trigger_type+jsonb). Decisiones cerradas: crear-desde-plantilla, cron */15, umbrales editables por regla, tope global 3. **NADA vivo el fin de semana; SEMANA10 programada (lunes 6–12).**
>
>
> **(RECON F8 ADELANTADO — exploración de esta sesión, sin construir).** **(RECON F8 ADELANTADO — publicación de promos/ads a plataformas + RRSS, exploración de esta sesión, sin construir):** DATO de Julio: trabajó con Pleez y le pidieron CREDENCIALES de Glovo/Uber → Pleez opera por VÍA B (automatización sobre los paneles de partner con las credenciales del cliente, como tspoon). **UBER EATS: vía A EXISTE** — Marketplace API con Promotions API suite + Uber Ads APIs (lanzadas jun-2026, programáticas, para POS/agregadores/integradores que gestionan ads en nombre de restaurantes + Reporting APIs); peaje = developer account + NDA/licencia + aprobación de partner. **CORREO ENVIADO a Uber** solicitando alta como INTEGRATION PARTNER (Marketplace + Promotions + Ads) — esperando respuesta 2-3 días. **GLOVO: sin API pública de promos** — su Partner API cubre solo pedidos/stock; promos y ads se gestionan en el Manager Portal (Promociones + Publicidad); los T&C prohíben ceder credenciales a terceros (→ la vía Pleez opera en zona gris del sector). RECON en vivo con credenciales de Llorente29: catálogo de tipos COMPLETO (Ofertas flash/Todo el menú/Productos seleccionados/Gastos de envío/2x1 — Folvy los cubre TODOS) + Publicidad (Posicionamiento inteligente + palabras clave, con informes) + informe de rendimiento que muestra el dato ORO: Llorente29 lleva **8.043€ invertidos / 29.657€ ventas / ROI 3,69x "Bueno" según Glovo — pero sobre BRUTAS** (sin restar descuento+comisión+coste = el cruce que solo Folvy puede hacer con escandallo+menu_item_channel_economics). Julio CREÓ una promo real por el flujo (Programada 06/07, luego CANCELADA): endpoint identificado `POST .../deals`, ANTI-BOT activo ("mantén pulsado, eres humano" → el replay pelado da 403; la vía B real = navegador gestionado con fingerprint, el foso de Pleez), y CONTRATO JSON capturado (globalEntityId GV_ES, dealTemplateId por tipo, discountPercentageRewardBehavior, productMenuSelectionBehavior.products[], selectedOutlets, dayOfWeekMatrixScheduleBehavior, sponsorship.ratios, start/endDate) → **el mapeo Folvy→Glovo es casi 1:1 con el modelo de campañas de esta semana** (value/scope/weekdays/time/calendario). **RRSS**: Meta Marketing API + TikTok Ads API son PÚBLICAS y robustas → automatización real; medición vía píxel/Conversions API → Shop → margen real por campaña (post-F5). Plan F8 SIN condicionar: 3 vías en paralelo (Uber API si responde · Glovo navegador gestionado con riesgo T&C declarado · RRSS API tras F5); las campañas ya nacen con channels[]. **PENDIENTE HubRise (cliente 2):** correo a Janaina preparado (arrancar cliente 2 Uber+JE en producción + plazos + billing test + pregunta de promos) EN ESPERA DELIBERADA 2-3 días hasta ver respuesta de Uber (para escribirlo con el alcance correcto); disparador = respuesta de Uber o máx. lunes/martes. **REGLA DE PITCH (Julio): al hostelero se le habla de QUÉ GANA (margen real vs el "ROI bueno" ciego de Glovo), NO de endpoints/anti-bot; de Glovo/Uber en futuro cercano sencillo, de canal propio en presente.** FRENTES FUTUROS DECLARADOS esta sesión: (1) **manual de usuario del módulo Ofertas** (comercial/onboarding, cuando el módulo estabilice o antes si se pide para ventas); (2) **i18n — internacionalización** (interfaz vía react-i18next + datos de BBDD name_es/en; la web pública EN/ES ya marca el patrón; frente propio con RECON); (3) **rediseño web pública folvy.app** (desfasada a jun-2026, no cuenta CRM/ofertas/dashboard/tienda; arranca por ESTRATEGIA DE MENSAJE con Claude coordinador → Claude Design maquetas → Claude Code implementa en folvy-landing).
>
> **SIGUIENTE del módulo tras G2d:** (campaign_rule + evaluador cron; disparadores franja/valle, caída de demanda y marca floja contra histórico PROPIO, clima Open-Meteo, eventos; origin='rule') → deuda catálogo↔escandallo (420/582 sin coste, lo que convierte el impacto de margen de parcial a total) → F5 post-pedido (email Resend sin esperar WhatsApp; absorbe cumpleaños+referidos). **PENDIENTES menores:** ciclo real con cocina abierta (chip dirección T1 + ganar→canjear→reset T3), KPI "Agotados" cuenta el espejo en espera como agotado, normalizar el scope del picker (categoría marcada no debe persistir también los hijos), precio Tequeños quedó 9,91€ (¿céntimo arriba tras la prueba del historial? revisar), regenerar database.ts, estilos muertos del checkout (couponToggle/couponError/welcomeBanner), F8 RECON de mecanismos de publicación (API vs credenciales de partner) antes de prometer push/ads de plataforma, decisión cashback (F6, de Julio). SEMANA10 queda Programada 6–12 jul (campaña real de Julio).
