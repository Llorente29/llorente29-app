...
**Última actualización: 2026-06-09 (CIERRE jornada). Lo último — BUG ESTRUCTURAL DEL ROUTER ARREGLADO (afectaba a TODA la app): F5/recarga en cualquier pantalla expulsaba a Inicio. Causa raíz: `isShellRoute` en `src/routes.ts` quedó del modelo viejo (solo reconocía `/shell`), pero el Shell vive en la RAÍZ desde G-8 (sin slug); los 3 useEffect de sincronización de slug en AppContext no frenaban → reescribían la URL con slug obsoleto en cada F5 → Inicio. FIX: `isShellRoute` ahora devuelve true para toda ruta autenticada que no sea auth-pública ni admin (commit 3bc2705). Verificado en vivo: F5 se mantiene en cualquier pantalla. ADEMÁS (mismo commit): CREAR PLATO DESDE VENTA HUÉRFANA — RPC `create_dish_from_unmapped` (venta del TPV sin plato en Folvy → recipe_item dish + lastapp_product_map + menu_item por marca + recast + devuelve id), con modal de confirmación Folvy (`ConfirmDialog` nuevo, reutilizable; sustituye al window.confirm nativo) y navegación a la ficha del escandallo (ruta absoluta `/kitchen/recetas?recipe=ID`, que sobrevive al remontaje; KitchenRecipesPage lee `?recipe=` con useSearchParams). ANTES (misma sesión, 4 frentes cerrados): (1) MOTOR DE CONSUMO TEÓRICO — ventas×escandallo → stock_movement 'consumo' (qty negativa, unit_cost sellado), motor puro SQL, paridad consumo==coste al céntimo, pestaña Consumo en InventoryPage, paso 4 en el webhook (commit af85058). (2) IDENTIDAD DEL RECAST JUBILADA + reprocess_sale — recast casa por canónico (ya no lee raw_products), reprocess_sale limpia consumo fósil antes de re-adaptar, idempotente (1511 movimientos en 2 corridas, 0 fósiles); casado canónico 329 product/0 no_brand/0 ambiguous/20 no_recipe (commits 6e7f765). (3) RECOSTEO + POLÍTICA DOS RELOJES — diagnóstico: no hay ventas mal costeadas; NULL/0 son legítimos (sin escandallo, reventa sin precio, combos por componentes). MODELO DOS RELOJES confirmado como decisión de producto: sale_line.computed_cost CONGELADO (margen histórico inmutable) / recipe_item.computed_cost VIVO (consumo/previsión). Sin código. (4) CAPA 4 FIABILIDAD — ya estaba hecha (08/06), VERIFICADA en vivo: 95,02% (5979,40€ casado / 6293€, 329/349 líneas, 313,60€ no_recipe). LECCIONES REFORZADAS: nunca `Set-Content -Encoding UTF8` sobre ficheros con acentos (corrompe tildes/ñ → usar Copy-Item del descargado, respeta encoding); nunca window.confirm (cartel de desarrollador → ConfirmDialog Folvy); RECON de la fuente primaria antes de tocar (el bug del F5 estaba en routes.ts, no donde parecía). Ver §1.1.**
