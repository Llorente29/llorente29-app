...
2. **INVENTARIO PERPETUO (EL TRONCO del MRP II) — SIGUIENTE FRENTE.** Entradas (recepción, ya escribe stock_movement+snapshot WAC) − salidas (consumo por ventas×escandallo) = stock en tiempo real por local. Conteos, mermas, caducidades, FEFO. De aquí cuelga balance/reorden/AvT real. Habilita el AUTOINVENTARIO IA.
...
7. Seguridad: rotar service_role/webhook tokens (Last `247ef137-...`). Code-splitting (~719KB gzip). 34 platos needs_review. Medidor coste IA por cuenta.
