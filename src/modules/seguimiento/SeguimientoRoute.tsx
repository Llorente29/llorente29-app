    ...
    <div className="fixed inset-0 bg-zinc-50 text-zinc-900 flex flex-col">
      ...
      <div className="flex-1 min-h-0 mx-4 rounded-2xl overflow-hidden ring-1 ring-zinc-200 relative">
        {MAPBOX_TOKEN
          ...
          : <div className="absolute inset-0 grid place-items-center text-sm text-zinc-500 p-6 text-center">{direccionParaMostrar(data.delivery_address) ?? 'Tu direccion'}</div>}
      </div>
      ...
      <div className="shrink-0 p-4 space-y-3">
        ...
        {data.delivery_address && <p className="text-xs text-zinc-400 text-center">Entrega en {direccionParaMostrar(data.delivery_address)}</p>}
      </div>
    </div>
  )
}
