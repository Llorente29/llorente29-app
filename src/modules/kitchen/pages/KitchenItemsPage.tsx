...
// V1: lista + alta. Sin edición todavía (create-then-edit en una sesión futura).
//
// Carga en paralelo recipe_item(raw) + kitchen_unit (la unidad base de cada
// ingrediente). El alta llama a createRecipeItem (que internamente dispara
// kitchen_recompute_item; aquí siempre saldrá === fixedCost porque un raw
// sin recipe_line es coste plano).
//
              ...
              <tbody>
                {items.map(item => {
                  ...
                  return (
                    <tr
                      key={item.id}
                      className="border-b border-border-default last:border-0 hover:bg-accent-bg transition-base"
                    >
                      <td className="p-3">
...
// ─────────────────────────────────────────────────────────────────────
// Modal inline de creación. Campos mínimos: nombre + unidad + coste fijo.
// Patrón "create-then-edit": el resto de campos se rellenarán cuando
// llegue una página de detalle (sesión futura).
// ─────────────────────────────────────────────────────────────────────
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-border-default">
          <h3 id="create-ingredient-title" className="text-base font-medium text-text-primary">
            Nuevo ingrediente
          </h3>
          <button
            type="button"
