// src/modules/multitenancy/services/brandLogoService.ts
//
// Logo por MARCA (no por cuenta). Una marca virtual (p.ej. Mila's) tiene su
// logo propio, distinto del de la empresa fiscal (accounts.logo_url). Vive en el
// bucket público `brand-logos` y su URL se persiste en `brand.logo_url`. Lo
// consume el ticket de bolsa de impresión (cabecera) y cualquier pantalla que
// muestre la marca.
//
// Calcado de accountLogoService con UNA diferencia de path: el fichero cuelga de
// {accountId}/{brandId}/ (accountId PRIMERO) para reusar las funciones RLS
// existentes (belongs_to_account / current_user_is_admin_or_manager_of sobre la
// 1ª carpeta = accountId), sin inventar un belongs_to_brand. Por eso upload/delete
// reciben (accountId, brandId).
//
// Proceso en cliente + bucket público, con tres rasgos heredados:
...
//     transparente y recorta el aire sobrante, para que "flote" sin marco.
//     Conservador: solo actúa si las 4 esquinas coinciden; nunca toca logos con
//     foto/degradado de fondo (no adivina).
//  3) Persiste la URL en brand.logo_url y borra el logo anterior al cambiarlo.
//
// RLS del bucket: SELECT belongs_to_account; INSERT/UPDATE/DELETE
// current_user_is_admin_or_manager_of(accountId). El path empieza por
// {accountId}/ para que la política resuelva la cuenta.
...
/** Lee la URL del logo actual de la marca (o null). */
export async function getBrandLogoUrl(brandId: string): Promise<string | null> {
  if (!supabase) throw new Error('Supabase no disponible')
  const { data, error } = await (supabase as any)
    .from('brand').select('logo_url').eq('id', brandId).single()
  if (error) throw new Error(`No se pudo leer el logo: ${error.message}`)
  return (data?.logo_url as string | null) ?? null
}

/** Sube un nuevo logo de marca: rasteriza + autotrim, sube al bucket, persiste la
 *  URL en brand.logo_url y borra el anterior. El path es {accountId}/{brandId}/...
 *  (accountId primero) para que la RLS resuelva la cuenta. Devuelve la URL pública. */
export async function uploadBrandLogo(accountId: string, brandId: string, file: File): Promise<string> {
  if (!supabase) throw new Error('Supabase no disponible')

  let previousUrl: string | null = null
  try { previousUrl = await getBrandLogoUrl(brandId) } catch { /* sigue */ }

  const blob = await rasterizeToPng(file)
  const ts = Date.now()
  const path = `${accountId}/${brandId}/logo-${ts}.png`
  ...
  const { error: updErr } = await (supabase as any)
    .from('brand').update({ logo_url: publicUrl }).eq('id', brandId)
  if (updErr) {
    await supabase.storage.from(BUCKET).remove([path]).catch(() => {})
    throw new Error(`No se pudo guardar el logo en la marca: ${updErr.message}`)
  }
...
/** Quita el logo de la marca: borra el fichero y limpia brand.logo_url. */
export async function deleteBrandLogo(brandId: string): Promise<void> {
  if (!supabase) throw new Error('Supabase no disponible')
  let current: string | null = null
  try { current = await getBrandLogoUrl(brandId) } catch { /* sigue */ }

  const { error: updErr } = await (supabase as any)
    .from('brand').update({ logo_url: null }).eq('id', brandId)
