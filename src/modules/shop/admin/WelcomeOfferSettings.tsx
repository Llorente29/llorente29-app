// src/modules/shop/admin/WelcomeOfferSettings.tsx
//
// F3 sub-paso 5 — Configuración de la oferta de BIENVENIDA por cuenta, con el
// IMPACTO DE MARGEN REAL delante (golpe nº1 de Folvy). El operador elige el valor
// (20% o 4€…) y ve, en vivo, qué le hace a su margen antes de guardar. Presets
// rápidos para comparar "20% vs 4€" sobre su pedido medio real.
//
// Autocontenida (estilos inline): se monta con <WelcomeOfferSettings accountId={…} />.
// Todo el cálculo y el guardado son server-side (preview_coupon_impact /
// save_welcome_offer); el front no calcula margen ni escribe el cupón a mano.

import { useEffect, useRef, useState, type CSSProperties } from 'react'
import {
  getWelcomeOffer, previewCouponImpact, saveWelcomeOffer,
...
export default function WelcomeOfferSettings({ accountId }: { accountId: string }) {
  ...
  useEffect(() => {
    let alive = true
    ;(async () => {
      const off = await getWelcomeOffer(accountId)
      if (!alive) return
      setActive(off.active); setDiscountType(off.discountType); setValue(off.value)
      const imp = await previewCouponImpact(accountId, off.discountType, off.value)
      if (!alive) return
      setImpact(imp); setFloorPct(imp?.floorPct ?? null); setLoading(false)
    })()
    return () => { alive = false }
  }, [accountId])

  // Recalcular impacto al cambiar tipo/valor (debounce ligero).
  useEffect(() => {
    if (loading) return
    setImpactBusy(true)
  ...
  function applyPreset(t: DiscountType, v: number) {
    setDiscountType(t); setValue(v); setSaved(false)
  }

  async function onSave() {
    setSaving(true); setError(null); setSaved(false)
    const res = await saveWelcomeOffer({ accountId, active, discountType, value, floorPct })
    setSaving(false)
  ...
  if (loading) {
    return <div style={{ ...s.card, color: C.inkFaint }}>Cargando la oferta de bienvenida…</div>
  }
  ...
  return (
    <div style={s.card}>
      <div style={s.head}>
        <div>
          <div style={s.title}>Oferta de bienvenida</div>
          <div style={s.sub}>El descuento del primer pedido, a cambio de unirse al Club. Elige el valor y mira su impacto en tu margen antes de guardar.</div>
        </div>
        <label style={s.switchRow}>
          ...
          <span style={s.switchLabel}>{active ? 'Activa' : 'Desactivada'}</span>
        </label>
      </div>

      {/* Presets rápidos: comparar 20% vs 4€ sobre el pedido medio real */}
      <div style={s.presets}>
        <span style={s.presetsLabel}>Prueba rápida:</span>
        <button type="button" style={s.presetBtn} onClick={() => applyPreset('percent', 10)}>10%</button>
        <button type="button" style={s.presetBtn} onClick={() => applyPreset('percent', 20)}>20%</button>
        <button type="button" style={s.presetBtn} onClick={() => applyPreset('fixed', 4)}>4 €</button>
        <button type="button" style={s.presetBtn} onClick={() => applyPreset('fixed', 5)}>5 €</button>
      </div>
      ...
      <div style={s.row}>
        ...
        <div style={s.field}>
          <label style={s.label}>Suelo de margen <span style={s.opt}>(opcional)</span></label>
          <div style={s.valueRow}>
            <input type="number" min={0} max={100} step={1}
              value={floorPct ?? ''} placeholder="sin suelo"
              onChange={(e) => { const v = e.target.value; setFloorPct(v === '' ? null : (parseFloat(v) || 0)); setSaved(false) }}
              style={s.input} />
      ...
      <div style={{ ...s.impact, opacity: impactBusy ? 0.6 : 1 }}>
        ...
        {discountType === 'fixed' && impact?.effectivePct == null ? (
          ...
          <>
            ...
            {afterBad && (
              <div style={s.warn}>
                El margen medio quedaría por debajo de tu suelo. Puedes guardarlo igual (la bienvenida se permite como inversión de captación), pero conviene revisarlo.
              </div>
            )}
          </>
        )}
...
const styles: Record<string, CSSProperties> = {
  ...
  presets: { display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14, flexWrap: 'wrap' },
