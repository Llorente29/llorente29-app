...
- **🔴 REGLA 12 — SE VERIFICA EL EFECTO, NUNCA QUE EL COMANDO PASÓ** *(02/09)*. La ausencia de error no es evidencia de nada: es la ausencia de error. `has_table_privilege` después del `revoke`; `pg_get_functiondef` después del deploy; `count(*)` después del insert.
  - *Cuatro casos reales, tres de ellos de la misma noche:* el **despliegue** que termina en 8 s sin desplegar nada *(frente 11)*; **`tsc --noEmit`** con exit 0 sin mirar una línea *(frente 20)*; un **`REVOKE` sin ser concedente**, que no falla y no retira nada *(frente 24)*; y el **editor SQL de Supabase**, que se traga sentencias y devuelve «Success» *(ya escrito arriba, desde julio)*.
  - *El detector:* si para saber si funcionó hay que fiarse de que no salió un error, no se ha verificado nada. Preguntar por el estado, no por el resultado de la orden.
