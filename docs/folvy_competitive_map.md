...
| Reparto propio (last-mile, coste real) | nadie lo integra en el ciclo de coste | **CONSTRUIDO Y VALIDADO EN SANDBOX (25/06)**: despacho a Catcher (`catcher-dispatch` auth+create order v1, envío real aceptado) + webhook de estado/rider/coste real + disparo automático por trigger; transversal, solo despacha donde Last no lo hace; el `transport_price` real entra al pedido | 🟢 golea (nadie mete el coste real de reparto propio en el ciclo) — falta tráfico real y registrar webhook en Catcher; + TRACKING AL CLIENTE (T4, 18/07) construido y en producción: página /seguir con moto en vivo + stepper + ETA + nombre/teléfono del rider (RPC track_by_token); notificación por WhatsApp en alta (Meta Cloud API directa, bloqueada por restricción de web del Business Manager de FoodInt) |
...
| Publicación automática en Uber | 🔴 | 🔴 | 🔴 | ⚪ | 🟢 **VIVO (10/07) por ROBOT DE NAVEGADOR** (gemelo de Glovo, `uber-arm.mjs` Playwright): la API oficial sigue bloqueada (`invalid_scope`=producción no aprobada) → pivote al brazo de navegador. **6 marcas propias publicadas de punta a punta, por local, verificadas** (asistente «% de ahorro en artículos» entero: locales+platos+%+fechas+crear). Targeting por local vía la calle en la etiqueta de la tienda (=`glovo_pos_hint`). Falta la última pieza: `end` por captura-de-ID + paridad UX (Uber a `ARMED_PLATFORMS`). Doc: `claude/folvy_uber_robot_estado.md` |
...
**Actualización 10/07 (MARATÓN TEAM — el frente pasó de 🔴 a 🟡/🟢 en varias filas):**
- **Informes de Team v2 → 🟢** (la ventaja estructural ya es real, no latente): comparativa por local con **% personal/ventas desde nóminas REALES** (no estimado), ticket medio, ventas/hora, franjas horarias (sobre-dimensionado en ámbar). Ventas+personal+coste laboral real en una sola verdad. Ningún vertical (Combo/Skello/Sesame/Factorial) tiene el coste laboral real cruzado con ventas reales sin integración frágil.
- **Nóminas → 🟢**: extractor IA + ingesta Gmail + adjunto a ficha; el coste laboral por empleado sale de la nómina definitiva, no de una estimación.
- **Cuadrante / planificación de turnos → 🟡** (ya no básico): rejilla empleados×días con áreas de color configurables por cuenta, **coste en vivo** al editar, **demanda en platos/hora** superpuesta (termómetro por día + curva horaria). Falta drag-and-drop fino y auto-generación.
- **Previsión de demanda para dotar → 🟢 en el ángulo que importa**: estudio de 3 años (28.685 tickets) → modelo jerárquico (prior general por tipo de negocio + coeficientes propios por cuenta, arranque en frío resuelto). **Benchmark Workforce.com** (líder, "industry-leading labor forecasting" con histórico+clima+tendencias de miles de locales; cobra ~35-42k$/año a 100 empleados; ROI Forrester 455%) y 7shifts/R365: Folvy **iguala su base** (día-tipo+estacionalidad+tendencia) con datos propios y **golea en dos ejes que su tamaño no les da**: (1) demanda en **platos reales de cocina**, no en euros (un plato es carga de trabajo, un euro no); (2) **ciclo cerrado** con escandallo+coste real de nóminas+margen (pueden recomendar recortar un turno con margen real, no estimado). **Honestidad declarada**: el clima/eventos (donde Workforce.com presume) están **apagados a propósito** hasta que Folvy tenga 20-30 locales — con 1 operación no hay muestra para calibrar el clima; ellos funcionan porque agregan miles de locales. Cuando Folvy escale, tendrá la misma arma con mejor materia prima.
- Siguen 🔴: anti-fraude real (QR dinámico), ausencias/vacaciones conectadas al cuadrante, app del empleado, auto-generación de cuadrante desde previsión (Capa 4, siguiente).
**Veredicto actualizado (10/07): 🟡 con dos goleadas ya reales (informes con coste real + previsión en platos).** El módulo Team pasó de ~1/10 a una base sólida; el ángulo ganador (margen/coste real donde otros estiman) ya está materializado en informes y previsión, no solo declarado.
