...
> **Motor de ofertas por plataforma (auditado 16/06 — NO es goleada, deuda-0):** YA EXISTE. **Pleez** (trypleez.com, Madrid, fundada 2020, Buenavista Equity) hace ofertas por canal con **push de 1 clic a Uber Eats/Glovo/Deliveroo**, guardarraíles de margen, disparadores por reglas, **clima + eventos deportivos** y competitor/price tracker (opera vía credenciales del restaurante + scraping de escaparates públicos; no API oficial). **Sapaad** = promos margen-multicanal en tiempo real; **Nory** = clima+eventos para forecasting. **Decisión: NO clonar** (océano rojo, incumbente local financiado). El único ángulo de Folvy = la VERDAD DE MARGEN (escandallo al céntimo + economía reconciliada de la factura, ponderada por mix, agnóstica de TPV) que Pleez no puede calcular → quedarse con el **guardarraíl de margen real por plato×plataforma**; Pleez = posible integración, no enemigo. Doc: `docs/folvy_motor_ofertas_diseno.md`.
