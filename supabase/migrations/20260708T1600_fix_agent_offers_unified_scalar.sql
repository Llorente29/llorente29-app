-- 20260708T1600_fix_agent_offers_unified_scalar.sql
--
-- FIX: agent_offers_unified reventaba con "cannot extract elements from a scalar"
-- cuando coupon.scope->'location_ids' (o 'brand_ids') es un JSON `null` en vez de un
-- array. Ocurre con las ofertas item_percent de Shop (pieza 1 v3), que son POR MARCA
-- y guardan scope.location_ids = null. `coalesce(x, '[]')` NO reemplaza el JSON null
-- (solo el NULL de SQL) → jsonb_array_elements_text sobre un escalar `null` explota.
--
-- Solución: guard con jsonb_typeof(...) = 'array' (robusto ante cualquier forma de
-- scope). No toca datos: las 17 ofertas ya están bien creadas. Solo la lectura.
--
-- CREATE OR REPLACE (DDL puro, sin SELECT de prueba dentro) → seguro en SQL Editor
-- pese a ser SECURITY DEFINER. Los GRANT se conservan al reemplazar.
