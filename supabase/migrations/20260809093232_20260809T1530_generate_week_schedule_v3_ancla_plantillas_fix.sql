-- 20260809T1530_generate_week_schedule_v3_ancla_plantillas.sql  (CORREGIDA)
-- FIX 09/08/2026: en PL/pgSQL `SELECT ... INTO` no admite un elemento de array
-- como destino (`into v_day_total[d+1]` → error 42601). Se pasa por la variable
-- escalar v_tmp y luego se asigna. Único cambio respecto al fichero de Code.
...
begin
  ...
  if position('o_shift_template_id' in v_def) = 0 then
    raise exception 'FALLO: generate_week_schedule no tiene o_shift_template_id';
  end if;
