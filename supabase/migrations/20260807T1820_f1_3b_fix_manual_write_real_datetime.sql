-- Aplicada: 2026-08-07 por MCP. F1.3b · Tapa el bug del alta/edición manual de fichajes:
-- grababan real_datetime=now() (o no lo tocaban). Un fichaje manual/corregido: la hora tecleada
-- ES la hora real -> real_datetime = esa hora. Arregla add_manual_clock_entry, edit_clock_entry,
-- resolve_clock_correction (ramas add y edit). Copia fiel de la definición viva.

CREATE OR REPLACE FUNCTION public.add_manual_clock_entry(p_employee_id uuid, p_type text, p_datetime timestamp with time zone, p_reason text, p_actor_label text DEFAULT NULL::text)
...
CREATE OR REPLACE FUNCTION public.edit_clock_entry(p_entry_id uuid, p_datetime timestamp with time zone, p_reason text, p_type text DEFAULT NULL::text, p_actor_label text DEFAULT NULL::text)
 RETURNS clock_entries
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.resolve_clock_correction(p_request_id uuid, p_approve boolean, p_note text DEFAULT NULL::text, p_actor_label text DEFAULT NULL::text)
 RETURNS clock_correction_request
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
