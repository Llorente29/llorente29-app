...
-- T4: aplica los alérgenos leídos de una ficha (compliance_document) al INGREDIENTE
-- (recipe_item raw) que respalda, con source='manual' + source_document_id, crea el
-- enlace ficha↔ingrediente y recomputa los platos/recetas que usan el ingrediente.
-- "IA propone, humano decide": esto se llama SOLO tras confirmación del usuario.
-- Guardarraíl admin/manager + aislamiento de cuenta. Solo toca los códigos declarados.

create or replace function public.apply_compliance_doc_allergens(
  p_document_id    uuid,
  p_recipe_item_id uuid,          -- el ingrediente (raw) al que respalda la ficha
  p_contains       text[],
  p_may_contain    text[]
) returns jsonb
language plpgsql
...
begin
  ...
  -- 4) Recomputar todos los platos/recetas que usan el ingrediente (herencia fill-only)
  for v_dish in
