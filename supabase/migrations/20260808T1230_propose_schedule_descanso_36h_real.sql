-- Aplicada: 2026-08-08 por MCP. Version VIVA de propose_schedule (verificada con
-- pg_get_functiondef el 08/08) — supersede a T1200 y T1210 en este mismo punto.
--
-- Sustituye el contador "max 5 dias" (incorrecto: quitaba media jornada legal)
-- por la regla real del art. 37.1: 36 h continuas de descanso en la semana.
-- Permite 6 dias trabajados si el hueco de 36 h existe.
...
declare
  ...
  v_horas     jsonb := '{}'::jsonb;
  v_turnos    jsonb := '{}'::jsonb;   -- employee_id -> [[ini,fin], ...]
  v_fin       jsonb := '{}'::jsonb;   -- employee_id -> fin del ultimo turno
    ...
    for v_i in 1 .. r_turno.necesita loop
      ...
      select count(*),
             ...
             count(*) filter (where not en_vac and not ya_hoy
                                and not public.has_weekly_rest_36h(turnos, v_ini, v_fin_t, p_week_start)),
      ...
      for r_cand in
        ...
        where e.account_id = p_account and e.active
          ...
          and not (coalesce(v_asignadas -> (r_turno.fecha::text), '[]'::jsonb)
                   @> to_jsonb(e.id::text))
          -- DURA art. 37.1: 36 h continuas de descanso en la semana
          and public.has_weekly_rest_36h(
                coalesce(v_turnos -> e.id::text, '[]'::jsonb), v_ini, v_fin_t, p_week_start)
          -- DURA art. 34.3: 12 h entre jornadas
          and (
            (v_fin ->> e.id::text) is null
            or v_ini >= ((v_fin ->> e.id::text)::timestamp + interval '12 hours')
          )
          -- DURA: tope de jornada contratada
      ...
      if not v_encontrado then
        ...
        motivo_hueco := nullif(concat_ws(' · ',
          ...
          case when v_bloq_desc  > 0 then 'perderian el descanso de 36 h (art. 37.1): '||v_bloq_desc end,
