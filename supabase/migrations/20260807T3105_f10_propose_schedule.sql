-- Aplicada 2026-08-07 por MCP. Verificado sobre la semana 24/08 en Foodint Alcalá:
--   5 personas x 7 turnos, reparto equilibrado (28,5 a 38,5 h/semana), solo 4 avisos de preferencia rota.
--   Johanny excluido correctamente por vacaciones aprobadas (restricción dura respetada).
--
-- F10 · GENERADOR DE CUADRANTES.
--
-- PRINCIPIO (del encargo de 12 fases): el LLM NO asigna. Asigna un SOLVER determinista y EXPLICABLE.
-- Cada propuesta lleva su motivo, para que el encargado entienda y pueda discutirla.
--
-- RESTRICCIONES DURAS (nunca se rompen): vacaciones/bajas aprobadas · no duplicar a la misma persona en
--   dos turnos del mismo día.
-- PREFERENCIAS BLANDAS (decisión de Julio: "recomendado sí, obligatorio no"): la disponibilidad inferida.
--   Si hace falta romperla para cubrir el servicio, se propone igualmente PERO marcada con aviso y motivo.
--   Esto es lo contrario de los competidores, donde la disponibilidad bloquea y el manager acaba peleándose
--   con el software.
--
-- Prioridad (benchmark Shiftbase: "empieza por el día más difícil de cubrir"):
--   1) turnos con mayor cobertura requerida primero
--   2) dentro de cada turno: disponibles antes que no-disponibles, y entre ellos el que MENOS horas lleva
--      esa semana (reparto justo, evita horas extra concentradas).
--
-- BUG CORREGIDO EN ORIGEN: `time + interval '24 hours'` DA LA VUELTA AL RELOJ en Postgres
-- (00:15 + 24h = 00:15), así que los turnos que cruzan medianoche daban horas NEGATIVAS
-- (Corrido1 14:45-00:15 => -14,5 h). Correcto: operar sobre INTERVAL, no sobre TIME.
