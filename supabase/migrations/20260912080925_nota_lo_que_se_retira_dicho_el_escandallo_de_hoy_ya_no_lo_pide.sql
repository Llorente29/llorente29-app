-- ════════════════════════════════════════════════════════════════════════════
-- LO QUE SE RETIRA, DICHO · la nota «el escandallo de hoy ya no pide esto»
--
-- EL SILENCIO QUE CIERRA: un ingrediente que la venta tenia apuntado, que el
-- corte deja libre, y que el escandallo de HOY ya no pide. Sus movimientos se
-- borran y no se escribe nada en su lugar; hasta hoy tampoco quedaba nota.
-- Consumo que se retira sin decirlo: regla 8 y regla 30.
--
-- MEDIDO EL 12/09 ensayando el paso 4: seis ventas del 15 al 27/08 perdian su
-- unico renglon de Albahaca —11,765 g cada una, 70,588 en total, Carabanchel—
-- y no quedaba una sola nota. Ese ingrediente solo vive en `v_previos`; nunca
-- llega a `base`, asi que no entra en `plan` y ninguna de las dos ramas de
-- arriba lo mira. Es el mismo silencio que el agujero del NULO de esta manana,
-- con otro mecanismo.
--
-- LA NOTA NO INVENTA EL MOTIVO, y eso es a proposito. Las causas posibles son
-- tres —la receta cambio, la linea dejo de estar mapeada (D1), o un extra
-- anula la cantidad (el HAVING de `base`)— y distinguirlas pediria comparar
-- versiones de escandallo que NO EXISTEN: `recipe_item_version` tiene UNA
-- fila, de UNA ficha. Medido por Julio a las 10:00. Asi que la nota dice lo
-- que se sabe y dice tambien que no se puede saber cual.
--
-- LAS DOS REGLAS QUE SALEN DE AHI, y son de Julio:
--   · Hacia atras: no se reescribe mas historia de la que ya esta decidida, y
--     todo lo que se retire deja nota. Lo protegido por el corte, protegido.
--   · Hacia delante: guardar la version del escandallo con la que se consumio,
--     para que esta pregunta no vuelva. Apuntado, no es de hoy.
--
-- ENSAYADO ANTES DE APLICAR, en transaccion revertida, 0 fallos:
--   R1 md5 del fichero == instalado, 1 sola firma
--   R2 las SEIS ventas reales: 6 movimientos de Albahaca por -70,588 y CERO
--      notas pasan a 0 movimientos y SEIS notas «ya no lo pide», corte a NULO
--   R3 segunda pasada: siguen 6 notas, ni duplica ni revienta con 21000
--   P1..P5 la guarda de precio, intacta: nulo -> escribe con el escandallo;
--      negativo -> 0 y nota; cero -> escribe sin valorar; sano -> escribe;
--      50x -> frenado
...
BEGIN
  ...
  IF NOT v_void THEN
    ...
    -- ── LO QUE SE RETIRA, DICHO ──────────────────────────────────────────
    -- Un ingrediente que ESTA venta tenia apuntado, que el corte deja libre,
    -- y que el escandallo de HOY ya no pide: sus movimientos se han borrado
    -- ahi arriba y aqui no hay nada que escribir. Sin esta nota se iba en
    -- silencio, que es la regla 8 y la 30 otra vez.
    --
    -- MEDIDO EL 12/09, ensayando el paso 4: seis ventas del 15 al 27/08
    -- perdian su unico renglon de Albahaca —11,765 g cada una, 70,6 en
    -- total— y no quedaba una sola nota. Ese ingrediente solo vive en
    -- `v_previos`; nunca llega a `base`, asi que no entra en `plan` y
    -- ninguna de las dos ramas de arriba lo mira.
    --
    -- POR QUE PASA, y la nota no lo finge: no hay una sola causa. Puede ser
    -- que la receta cambiara, que la linea dejara de estar mapeada (D1), o
    -- que un `remove_item` anule la cantidad (el HAVING de `base`). Las tres
    -- dicen lo mismo de cara al almacen, y distinguirlas pide comparar
    -- versiones de escandallo que no existen: `recipe_item_version` tiene
    -- UNA fila, de UNA ficha. Asi que la nota dice lo que se sabe y no
    -- inventa el motivo.
    --
    -- `corte` va a NULO a proposito: aqui no hay corte que citar, y por eso
    -- la columna dejo de ser NOT NULL.
    --
    -- DISJUNTO POR CONSTRUCCION con las otras dos escrituras de la misma
    -- sentencia: `limpia` y `notas` solo tocan lo que esta en `items`, y
    -- esto solo toca lo que NO esta. Sin eso, dos filas con la misma clave
    -- rompen la venta entera con 21000.
    retirados AS (
      INSERT INTO public.sale_consumption_skip
        (account_id, sale_id, recipe_item_id, location_id, fecha_venta, corte, motivo)
      SELECT v_sale.account_id, p_sale_id, x.item, v_sale.location_id, v_fecha, NULL::timestamptz,
        ...
        FROM unnest(v_libres) AS x(item)
       WHERE NOT EXISTS (SELECT 1 FROM items i WHERE i.item = x.item)
      ON CONFLICT (sale_id, recipe_item_id) DO UPDATE
        SET corte = EXCLUDED.corte, fecha_venta = EXCLUDED.fecha_venta,
            motivo = EXCLUDED.motivo, created_at = now()
      RETURNING recipe_item_id
...
BEGIN
  ...
  SELECT md5(p.prosrc) INTO v_md5 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.proname='generate_sale_consumption';
  IF v_md5 <> 'b205e00cf30248f131d110ef12724005' THEN
    RAISE EXCEPTION 'md5 del escritor instalado = %, esperaba b205e00cf30248f131d110ef12724005', v_md5;
  END IF;
END $huellas$;