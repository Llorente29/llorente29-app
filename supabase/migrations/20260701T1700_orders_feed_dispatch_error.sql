-- supabase/migrations/20260701T1700_orders_feed_dispatch_error.sql
--
-- Añade dispatch_error al feed (para el botón Despachar/Reintentar en la tarjeta).
-- Igual que la versión previa del feed, sólo suma un campo más al SELECT.
--
-- Frente "reparto visible en el pedido": el feed de Orders expone los campos de
-- despacho que ya viven en `sale` (rellenados por catcher-webhook / brokers), para
-- que la tarjeta pinte la fila plegable de reparto (broker · estado · rider · tel).
--
-- La fila se muestra cuando hay carrier_code (reparto propio: Catcher/Jelp), venga
-- el pedido del canal que venga (Glovo/Uber/JE/Shop). Sin carrier propio, el front
-- decide por service_type ("lo lleva la plataforma" / recogida).
--
-- Solo se AÑADEN campos al SELECT de `vivos` y al JSON de `tickets`. El resto de la
-- función (líneas, hijas, notas, día de negocio) queda idéntico.
