-- supabase/migrations/20260622T1700_orders_feed_cerrados_dia_local.sql
-- ============================================================================
-- orders_feed: la pestaña "Cerrados" muestra TODO el día local, no solo 6h.
--
-- ANTES: los terminales (completed/rejected/cancelled/delivery_failed) se ocultaban
-- si su cierre era anterior a now() - 6h (ventana móvil). En Foodint Alcalá eso
-- escondía los cerrados antes de las 14h aunque fueran de hoy.
--
-- AHORA: la ventana de 6h se ELIMINA. Los vivos (no terminales) se muestran siempre.
-- Los terminales se muestran si su cierre cae en el DÍA EN CURSO [00:00, 24:00) en
-- la ZONA HORARIA de la cuenta (accounts.timezone, respaldo 'Europe/Madrid'), no en
-- UTC — si no, los pedidos de la noche se irían al día siguiente. sale.* es timestamptz
-- en UTC; calculamos la medianoche local y la devolvemos a timestamptz para comparar.
--
-- Único cambio vs 20260620T0100: +declares v_tz/v_day_start, +cálculo de la medianoche
-- local, y el WHERE de la CTE `vivos`. Todo lo demás (marca, familias, hijas con
-- group_type, notas, alérgenos, marcado, códigos de plataforma, orden) INTACTO.
-- Firma idéntica (orders_feed(uuid) returns jsonb) -> CREATE OR REPLACE, conserva grants.
-- security definer + belongs_to_account. Idempotente, sin BEGIN/COMMIT, sin SELECT de prueba.
-- ============================================================================
...
begin
  ...
  -- Medianoche LOCAL de hoy, devuelta a timestamptz (UTC) para comparar con sale.*.
  ...
  with vivos as (
    ...
    where s.location_id = v_location_id
      ...
      and (
        ...
        -- TERMINALES: solo si su cierre cae en el DÍA LOCAL en curso [00:00, 24:00).
        or (
