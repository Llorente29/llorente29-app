-- F1.5 (corrección) · Una 'entrada' solo cuenta si NO hay otra 'entrada' antes de su salida.
-- Sin este guard, una entrada huerfana (fichaje de salida olvidado) se emparejaba con una salida
-- posterior e inventaba horas: caso real 08/07 (entrada 10:30 + entrada 19:47) = +11,4 h ficticias.
-- El motor anterior lo descartaba exigiendo que el SIGUIENTE fichaje fuese salida; aqui se conserva esa
-- proteccion pero permitiendo pausas en medio.
...
as $function$
  ...
  valid as (
    ...
    where s.ended_at is not null
      ...
      -- entrada huerfana: hay otra 'entrada' antes de la salida -> se olvido fichar la salida. Se descarta.
      and not exists (
