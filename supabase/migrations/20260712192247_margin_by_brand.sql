...
language sql stable security invoker as $$
  with base as (
    select s.brand_id, b.name as brand, s.channel_id, s.location_id,
      sum(sl.quantity*sl.unit_price) as venta,
  ...
  bybrand as (
    select brand,
      ...
      round(sum(coalesce(food,0)) filter (where food is not null)) as food,
      round(sum(venta * coalesce(rate,0)/100.0)) as comision,
      round(100.0*sum(venta * coalesce(rate,0)/100.0)/nullif(sum(venta),0),1) as comision_pct,
      round(100.0*sum(food)/nullif(sum(venta) filter (where food is not null),0),1) as food_pct,
  ...
  select jsonb_build_object(
    'total', jsonb_build_object(
      'venta', round(sum(venta)),
      'comision', round(sum(comision)),
      'food', round(sum(food))
    ),
...
comment on function public.margin_by_brand is
  'Margen por marca con comision REAL (resolve_channel_commission, asume reparto plataforma) + food cost REAL (escandallo). Promo/personal se anaden en cliente como supuestos.';