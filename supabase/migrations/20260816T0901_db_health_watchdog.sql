...
--
-- Interpretación del umbral del encargo ("más de 3 procesos esperando lock
-- durante más de 2 min"): se cuenta cada backend de pg_stat_activity con
-- wait_event_type='Lock' cuyo query_start lleva más de 2 minutos — es decir,
-- procesos que llevan bloqueados ELLOS MISMOS más de 2 minutos, no "el conteo
-- total llevó 2 minutos por encima de 3". Si Julio quería la otra lectura,
-- decirlo y se ajusta: es un cambio de una línea en db_health_snapshot().
--
...
-- fichero (compiló, corrió, no dejó rastro) — regla "validar SQL por MCP antes
-- de entregar".
...
create table if not exists public.db_health_snapshot_log (
  ...
  waiting_locks_over_2min  int not null,
...
comment on table public.db_health_snapshot_log is
  ...
  'en cada ejecución de db_health_watchdog().';
...
-- ── 2) RPC de chequeo (puro, sin efectos secundarios) ─────────────────────────
...
begin
  ...
  return jsonb_build_object(
    ...
    'waiting_locks_over_2min', v_waiting_lock,
...
comment on function public.db_health_snapshot() is
  'Chequeo puro de salud de BBDD (conexiones, locks bloqueados >2min, tx más '
  'vieja). Sin efectos secundarios — el registro y el aviso los hace '
  'db_health_watchdog().';

-- ── 3) Vigía: registra + evalúa umbral + avisa ────────────────────────────────
...
begin
  begin
    ...
    v_waiting := (v_snap->>'waiting_locks_over_2min')::int;

    insert into public.db_health_snapshot_log
      (total_connections, waiting_locks_over_2min, oldest_tx_seconds)
    values
        ...
        if v_secret is not null then
          perform net.http_post(
            ...
            body    := jsonb_build_object(
              'subject', 'BBDD con bloqueos — ' || v_waiting || ' proceso(s) esperando lock >2min',
              'message', 'db_health_watchdog detectó ' || v_waiting
                         || ' proceso(s) en pg_stat_activity esperando un lock desde hace más de 2 minutos.' || chr(10)
...
comment on function public.db_health_watchdog() is
  ...
  'por system-alert si hay >3 procesos esperando lock >2min. Antiruido: no '
  'repite aviso en <15min mientras persiste. Nunca falla en silencio.';
