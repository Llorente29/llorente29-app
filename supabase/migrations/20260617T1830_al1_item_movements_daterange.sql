-- supabase/migrations/20260617T1830_al1_item_movements_daterange.sql
--
-- AL1 — El historial del artículo en la ficha se limita por fecha (por defecto
-- últimos 30 días, ampliable con filtro). Reescribe item_movements añadiendo
-- p_from / p_to (occurred_at). Mantiene p_limit como tope de seguridad.
...
-- Reemplaza la versión anterior (3 args). Quitamos la firma vieja para no dejar sobrecarga.
drop function if exists public.item_movements(uuid, uuid, int);

grant execute on function public.item_movements(uuid, uuid, timestamptz, timestamptz, int) to authenticated;
