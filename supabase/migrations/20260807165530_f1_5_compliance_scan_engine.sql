-- F1.5 / Pantalla Cumplimiento · Detector de infracciones de convenio sobre jornadas REALES.
-- Lee la politica efectiva (local > cuenta) y marca 4 tipos de incumplimiento, cada uno con su base legal.
-- Trabaja sobre fichajes reales, no sobre el contrato (contract_type esta a medias en los datos).
...
language sql stable
as $function$
  with pol as (  -- politica efectiva: la del local si existe, si no la de la cuenta
  ...
  -- 1) descanso obligatorio no disfrutado: jornada supera el umbral pero no hubo pausa registrada
  ...
  union all
  -- 2) mas horas seguidas de las permitidas sin pausa (Madrid: 5 h)
  select j.employee_id, j.emp_name, j.location_id, j.started_at, j.ended_at, j.minutes, j.night_min,
  ...
  union all
  -- 3) menos descanso que el minimo entre jornadas (ET 34.3: 12 h)
  select j.employee_id, j.emp_name, j.location_id, j.started_at, j.ended_at, j.minutes, j.night_min,
         'DESCANSO_ENTRE_JORNADAS', 'serious',
         'Solo '||round(extract(epoch from (j.started_at - j.prev_end))/3600.0,1)||' h desde la jornada anterior',
         'ET art. 34.3 (12 h)'
  from j
  ...
  union all
  -- 4) jornada diaria por encima del maximo (ET: 9 h ordinarias)
  select j.employee_id, j.emp_name, j.location_id, j.started_at, j.ended_at, j.minutes, j.night_min,
