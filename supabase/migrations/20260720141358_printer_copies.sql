...
as $function$
  select coalesce(jsonb_agg(jsonb_build_object(
           ...
           'ip', config->>'ip', 'port', coalesce((config->>'port')::int, 9100),
           'doc_types', to_jsonb(doc_types), 'copies', copies, 'is_active', is_active
         ) order by name), '[]'::jsonb)
  from printer where location_id = p_location_id and public.belongs_to_account(account_id);
$function$;
...
begin
  ...
  return (
    ...
    from printer where location_id = v_device.location_id and account_id = v_device.account_id
  );
end;
$function$;
...
create or replace function public.upsert_printer(
  ...
  p_transport text, p_config jsonb, p_doc_types text[], p_is_active boolean, p_copies int default 1)
...
begin
  ...
  if p_id is null then
    ...
    values (p_account_id, p_location_id, p_name, p_transport, coalesce(p_config,'{}'::jsonb),
            coalesce(p_doc_types, array['bag','kitchen','labels']), coalesce(p_is_active, true),
            greatest(1, least(9, coalesce(p_copies, 1))))
    returning id into v_id;
  else
    update printer set location_id = p_location_id, name = p_name, transport = p_transport,
      config = coalesce(p_config, config), doc_types = coalesce(p_doc_types, doc_types),
      is_active = coalesce(p_is_active, is_active),
      copies = greatest(1, least(9, coalesce(p_copies, copies))), updated_at = now()
    where id = p_id and account_id = p_account_id returning id into v_id;
    if v_id is null then raise exception 'upsert_printer: impresora % no encontrada en la cuenta', p_id; end if;
  end if;
  return v_id;
end;
$function$;
...
create or replace function public.upsert_printer_by_token(
  p_device_token text, p_id uuid, p_name text, p_config jsonb, p_doc_types text[], p_is_active boolean, p_copies int default 1)
 returns uuid language plpgsql security definer set search_path to 'public'
as $function$
...
begin
  ...
  if p_id is null then
    insert into printer (account_id, location_id, name, transport, config, doc_types, is_active, copies)
    values (v_device.account_id, v_device.location_id, p_name, 'escpos_network', coalesce(p_config,'{}'::jsonb),
            coalesce(p_doc_types, array['bag','kitchen','labels']), coalesce(p_is_active, true),
            greatest(1, least(9, coalesce(p_copies, 1))))
    returning id into v_id;
  else
    update printer set name = p_name, config = coalesce(p_config, config),
      doc_types = coalesce(p_doc_types, doc_types), is_active = coalesce(p_is_active, is_active),
      copies = greatest(1, least(9, coalesce(p_copies, copies))), updated_at = now()
    where id = p_id and account_id = v_device.account_id and location_id = v_device.location_id
    returning id into v_id;
    if v_id is null then raise exception 'upsert_printer_by_token: impresora % no encontrada en el local del dispositivo', p_id; end if;
  end if;
  return v_id;
end;
$function$;
...
begin
  ...
  for v_printer in
    ...
    where account_id = p_account_id and location_id = p_location_id and is_active and p_doc_type = any(doc_types)
  loop
...
begin
  ...
  if v_fire and not exists (select 1 from print_job pj where pj.sale_id = new.id and pj.source = 'auto') then
    begin
      for v_printer in
...
begin
  ...
  if not (public.current_user_is_admin() or public.current_user_is_admin_or_manager_of(v_acc)) then
    raise exception 'reprint_order: sin acceso a la cuenta';
  end if;
  for v_printer in
    select id, doc_types, copies from printer where account_id = v_acc and location_id = v_loc and is_active
  loop
    foreach v_doc in array v_printer.doc_types loop
      for v_i in 1..greatest(1, coalesce(v_printer.copies, 1)) loop
        insert into print_job (account_id, location_id, printer_id, sale_id, doc_type, payload, source, status)
...
begin
  ...
  if not exists (select 1 from sale where id = p_sale_id and account_id = v_device.account_id and location_id = v_device.location_id) then
    raise exception 'reprint_order_by_token: pedido no encontrado en el local del dispositivo';
  end if;
  for v_printer in
    select id, doc_types, copies from printer where account_id = v_device.account_id and location_id = v_device.location_id and is_active
  loop
    foreach v_doc in array v_printer.doc_types loop
      for v_i in 1..greatest(1, coalesce(v_printer.copies, 1)) loop
        insert into print_job (account_id, location_id, printer_id, sale_id, doc_type, payload, source, status)
