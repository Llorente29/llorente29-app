-- 20260814T1320_recepcion_oficina_a3_adjust_not_goods_entrada_tardia.sql
-- ENCARGO CODE (14/08) feat/recepcion-oficina-cierre, Tramo A.3 + A.3.bis
...
--
--   A.3  — parámetros nuevos p_not_goods/p_not_goods_kind (DEFAULT → no rompe
--          llamadas existentes). Si p_not_goods: reversa el movimiento previo
--          si lo hubiera, NO repostea, escribe not_goods/not_goods_kind.
--
--   A.3.bis — ¿existe algún stock_movement con source_type='goods_receipt_line'
--          y source_id = esta línea?
--            NO → ENTRADA TARDÍA: movement_type='recepcion',
--                 occurred_at = coalesce(v_receipt.received_at, now())
--            SÍ → CORRECCIÓN: reverso + reposteo, ambos 'ajuste', occurred_at=now() (como hoy)
--          Excepción de periodo cerrado: si hay un inventory_count de ese
--          location_id con closed_at no nulo posterior a la fecha del
--          albarán, entra a closed_at + 1 segundo (nunca dentro del periodo
--          ya asentado) y se devuelve closed_period_note para que el cliente
--          lo explique en pantalla sin jerga.
--
-- ⚠️ CREATE OR REPLACE con una lista de parámetros distinta crea una
-- SOBRECARGA nueva en Postgres, no sustituye la función — se DROPea
-- explícitamente la firma vieja de 6 argumentos para que solo quede una.
...
-- ── Sobrecarga vieja fuera — solo debe quedar la de 8 argumentos ─────────
drop function if exists public.adjust_goods_receipt_line(uuid, uuid, uuid, numeric, numeric, text);

-- ── Grants — mismo patrón que la firma anterior (verificado por MCP: el
--    objeto nuevo ya heredaba PUBLIC/anon/authenticated/service_role vía
--    default privileges, pero se deja explícito para un deploy limpio) ────
grant execute on function public.adjust_goods_receipt_line(
  uuid, uuid, uuid, numeric, numeric, text, boolean, text
