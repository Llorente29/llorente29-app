-- IVA 3/3 — preview_platform_promo_impact. Ver repo:
-- supabase/migrations/20260817T2358_iva_3_preview_platform_promo_impact.sql
--
-- La mas pequena: toda la cadena de abajo (base comisionable, ingreso neto,
-- margen) YA dividia bien. El unico error era pvp_cli, que multiplicaba por
-- (1+iva) una cifra que ya lo llevaba. Y se renombra price_sin_iva -> pvp_bruto:
-- el nombre afirmaba la interpretacion equivocada.
...
AS $function$
  ...
  items as (
    select m.id, m.name, b.name as brand_name,
           -- ANTES se llamaba price_sin_iva. menu_item.price lleva el IVA incluido.
...
'Prevision de impacto de promo de plataforma. menu_item.price es PRECIO CON IVA INCLUIDO (17/08): pvp_cli = coalesce(override, price) sin multiplicar. Toda la cadena de abajo ya dividia correctamente. price_sin_iva renombrada a pvp_bruto porque el nombre afirmaba la interpretacion equivocada.';