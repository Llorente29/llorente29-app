-- 20260626T1200_shop_brand_menu_by_slug.sql
-- Aplicada: (pendiente — SQL Editor)
-- Carta PÚBLICA de una marca para Folvy Shop (sin sesión). Espejo de
-- brandCatalogService.listCategoriesWithProducts pero por (slug de cuenta + brandId),
-- validando que la marca esté PUBLICADA en el hub. Solo platos disponibles
-- (is_available) y activos; agotados ocultos. Combos incluidos como un item más.

create or replace function shop_brand_menu_by_slug(p_slug text, p_brand_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = public
as $$
declare
  v_account_id uuid;
  v_brand record;
  v_cats jsonb;
begin
  select id into v_account_id from accounts where slug = p_slug;
  if v_account_id is null then
    return null;
  end if;

  -- La marca debe ser de esta cuenta y estar publicada/visible en el hub
  select b.id, b.name, b.logo_url, b.cuisine_code,
         st.accent_color, st.seed_rating, st.seed_rating_count
    into v_brand
  from brand b
  join shop_theme st on st.brand_id = b.id
  where b.id = p_brand_id
    and b.account_id = v_account_id
    and st.hub_visible = true
    and st.is_published = true;
  if v_brand.id is null then
    return null;
  end if;

  -- Categorías con sus platos disponibles (agotados ocultos)
  select coalesce(jsonb_agg(cat order by cat.position nulls last, cat.name), '[]'::jsonb)
  into v_cats
  from (
    select c.id, c.name, c.emoji, c.position,
           coalesce(jsonb_agg(
             jsonb_build_object(
               'id', mi.id,
               'name', mi.name,
               'description', mi.description,
               'photo_url', mi.photo_url,
               'price', mi.price,
               'product_type', mi.product_type
             ) order by mi.position nulls last, mi.name
           ) filter (where mi.id is not null), '[]'::jsonb) as products
    from menu_category c
    join menu_item mi
      on mi.menu_category_id = c.id
     and mi.account_id = v_account_id
     and mi.brand_id = p_brand_id
     and mi.is_active is not false
     and mi.is_available is not false
     and mi.archived_at is null
    where c.account_id = v_account_id
      and c.brand_id = p_brand_id
    group by c.id, c.name, c.emoji, c.position
    having count(mi.id) > 0
  ) cat;

  return jsonb_build_object(
    'brand_id', v_brand.id,
    'name', v_brand.name,
    'logo_url', v_brand.logo_url,
    'accent_color', v_brand.accent_color,
    'cuisine_code', v_brand.cuisine_code,
    'rating', v_brand.seed_rating,
    'rating_count', v_brand.seed_rating_count,
    'categories', v_cats
  );
end;
$$;

grant execute on function shop_brand_menu_by_slug(text, uuid) to anon, authenticated;
