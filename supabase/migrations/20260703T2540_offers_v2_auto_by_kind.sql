-- 20260703T2540_offers_v2_auto_by_kind.sql
-- Aplicada: (pendiente)
--
-- FIX G2·C2 — Automáticas de DISTINTO kind deben CONVIVIR. Requiere 2500/2510/2530.
--
-- BUG: coupon_account_one_auto = UNIQUE (account_id) WHERE (auto_apply AND active)
-- nació en F3 cuando la única automática era la bienvenida; con Ofertas v2 un
-- free_delivery auto choca con la bienvenida auto en la misma cuenta.
--
-- FIX (según el RECON):
--   1) Índice: un solo auto activo POR KIND por cuenta → UNIQUE (account_id, kind)
--      WHERE (auto_apply AND active). Protege la bienvenida (standard) y deja convivir
--      un free_delivery auto. (item_percent/frequency no usan auto_apply.)
--   2) place_shop_order: el free_delivery pasa a un LANE PROPIO que COEXISTE con el
--      cupón de subtotal (bienvenida/frecuencia/código). El de subtotal descuenta el
--      subtotal; el free_delivery descuenta el envío. La resolución principal EXCLUYE
--      kind='free_delivery' (nunca se mezclan en el mismo slot). Ambos se aplican y se
--      registran por separado; el total refleja los dos. Documentado.
--
-- Base = texto vivo de 20260703T2530. Cambios quirúrgicos. No se prueba en la tx.

begin;

-- ── 1) Índice único por kind ─────────────────────────────────────────────────
drop index if exists public.coupon_account_one_auto;
create unique index coupon_account_one_auto_per_kind
  on public.coupon (account_id, kind)
  where (auto_apply and active);

-- ── 2) place_shop_order: free_delivery en lane propio (coexiste) ─────────────
...
declare
  ...
  -- G2c/C2: free_delivery en lane propio
  v_fd           coupon%rowtype;
...
begin
  ...
  -- Resolución del cupón de SUBTOTAL (excluye free_delivery: va en su lane).
  ...
  -- ── G2·C2: ENVÍO GRATIS en lane propio (COEXISTE con el cupón de subtotal) ──
  -- Resuelve por código (si el código tecleado es un free_delivery) o por auto_apply.
  select * into v_fd
  from coupon
  where account_id = v_acc and active and kind = 'free_delivery'
  ...
  -- coupon_json: refleja el cupón de subtotal + la marca de envío gratis.
  if v_coupon.id is null and v_fd_applied then
    -- Solo envío gratis (auto o por código): que la píldora/línea lo muestre.
    v_coupon_json := jsonb_build_object(
  ...
  -- ── Canje del ENVÍO GRATIS (lane propio; is_cycle=true, exento del único) ──
  if v_fd.id is not null and v_fd_discount > 0 then
    begin
      ...
      values (
        ...
        round(v_fd_discount,2), round(v_subtotal,2), null, true);
    exception when others then null;
    end;
  end if;
