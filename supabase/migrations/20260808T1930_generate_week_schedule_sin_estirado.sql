-- ============================================================
-- Rescatada del historial vivo de Supabase (aplicada 08/08/2026 17:07,
-- nunca versionada hasta ahora). Migración 4/12 de la secuencia F10
-- T1900→T2010 que rediseñó el reparto de horas de generate_week_schedule
-- en caliente sobre producción en una sola tarde.
-- ESTADO: corrige el bug de T1920 — el estirado de bloques hacia demanda
-- partida (ej. abierto 13-16 y 20-23, cerrado 17-19) se solapaba y doblaba
-- las personas-hora necesarias. Usa bloques naturales, sin estirar; el
-- rediseño de capas y turno partido de T1920 se mantiene.
-- Versión buena de la secuencia: T2010 (quitar_pasada3_relleno).
-- Resumen completo: memoria project_f10_solver_legal.
-- ============================================================

-- BUG RAIZ ENCONTRADO: el CTE 'ext' estiraba CADA bloque hasta el "turno objetivo"
-- (~8 h) dentro de la ventana del dia. Con demanda partida (lunes: 13-16 y 20-23, con
-- el local cerrado 17-19), los dos bloques de 4 h se estiraban a 13-21 y 16-24 y quedaban
-- SOLAPADOS: 16 personas-hora para un dia que necesita 8. Al necesitar 2 personas por dia
-- donde bastaba 1, el presupuesto de horas se agotaba y lunes-jueves quedaban vacios.
--
-- ARREGLO: no estirar. Se usan los bloques naturales de demanda. Dos bloques del mismo
-- dia separados por el valle se asignan a la MISMA persona como turno partido (permitido
-- por Julio como recurso, con hueco >= split_min_gap y total diario <= jornada maxima),
-- que es exactamente lo que se hace hoy en el local.
-- Solo se parte un bloque cuando supera la jornada maxima.
...
begin
  ...
  for r_b in
    ...
    -- BLOQUES NATURALES: sin estirar
    blq as (select f,nivel,min(h) hi,max(h)+1 hf from grp group by f,nivel,g),
    -- solo se parte lo que supera la jornada maxima
  ...
  loop
    ...
    for r_c in
      ...
      -- prioridad: (1) quien ya trabaja ese dia -> completa su jornada partida antes de
      -- traer a otra persona; (2) quien mas lejos este de su jornada contratada
