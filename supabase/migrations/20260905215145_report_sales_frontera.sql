-- ── LA FRONTERA: autoriza, mide las dos ventanas y ESCRIBE lo que ha medido ──
--
-- Recibe las DOS ventanas ya calculadas por quien llama. No las deduce, y es
-- deliberado: `p_from − (p_to − p_from)` —lo que hace `sales_dashboard`—
-- desalinea una hora la semana del cambio de hora, y para «este mes» ni
-- siquiera da el mes anterior. El huso ya está resuelto, con doble pasada por
-- el cambio de hora, en `src/lib/fechas.ts`; se usa, no se reescribe en SQL.
--
-- Lo que SÍ hace la frontera es NEGARSE a comparar peras con manzanas: si las
-- dos ventanas no duran lo mismo (±1 h de margen por el cambio de hora), y no
-- se ha declarado que es un periodo de calendario, aborta. Nunca un periodo
-- parcial contra uno completo.
--
-- Y devuelve `meta` con el intervalo exacto en hora local, los filtros y la
-- regla de medida. Los cuatro errores de la semana fueron una cifra sin su
-- denominador o sin su regla al lado; aquí van pegados a la cifra, no en un
-- desplegable.
    ...
    'meta', jsonb_build_object(
      ...
      'cobertura_coste', jsonb_build_object(
        ...
        'sin_hueco_de_modificador', jsonb_build_object(
          ...
          'regla', 'Ademas de lo anterior, el pedido no lleva ningun modificador COBRADO sin impacto confirmado. Ese modificador mete su comida en el precio y no en el coste: el pedido cuenta como costeado y el margen sale mejor de lo que es.'),
...
comment on function public.report_sales(uuid, timestamptz, timestamptz, timestamptz, timestamptz, text[], uuid[], uuid[], text, uuid[], text[], boolean) is
  'Generador de informes: frontera que autoriza por cuenta, mide las dos ventanas con la misma regla y devuelve meta con el intervalo local, los filtros y la regla de medida.';
