...
-- nunca versionada hasta ahora). Migración 7/12 de la secuencia F10
-- T1900→T2010 que rediseñó el reparto de horas de generate_week_schedule
-- en caliente sobre producción en una sola tarde.
-- ESTADO: conecta la PASADA 3 (assign_extra_block, creada en T1950) dentro
-- de generate_week_schedule, ejecutando 3a "doblar en el pico" y 3b "abrir
-- 17-19 entre semana" justo después de cubrir la demanda (pasada 1). La
-- llamada a assign_extra_block quedó rota (recogía mal los OUT) y se
-- corrigió en la siguiente migración (T1970); esta pasada 3 fue retirada
-- por completo en T2010.
-- Versión buena de la secuencia: T2010 (quitar_pasada3_relleno).
-- Resumen completo: memoria project_f10_solver_legal.
-- ============================================================

-- Añade la PASADA 3 al generador: reparte las horas contratadas sobrantes
-- en el orden fijado por Julio: 3a doblar pico -> 3b abrir 17-19 -> 3c prep/limpieza.
...
AS $function$
declare
  r_b record; r_c record; r_x record; r_p record;
  ...
  v_found boolean; v_pass int; v_cambio boolean; v_ix int; v_ok boolean; v_falta numeric;
begin
  ...
  for r_p in
    ...
    having max(h)+1 - min(h) <= 4
    order by f
  loop
    select * into v_ok from public.assign_extra_block(p_account,p_location,p_week_start,
      r_p.f, r_p.hi, r_p.hf, 'refuerzo_pico',
      v_plan, v_horas, v_turnos, v_fin, v_max_h, v_rest_min, v_rest_12h, v_split_gap) x(a,b,c,d,e);
  end loop;

  -- ============ PASADA 3b: ABRIR 17-19 ENTRE SEMANA ============
  for r_p in
    select (p_week_start + d)::date f from generate_series(0,3) d
  loop
    select * into v_ok from public.assign_extra_block(p_account,p_location,p_week_start,
