-- 20260827214953_orders_feed_expone_platform_order_ref.sql
-- ============================================================================
-- El preview de ticket lee del feed, no de order_for_print
-- ============================================================================
-- Tercera de la tanda de platform_order_ref (20260827214035, ...214146). El
-- papel ya salia bien con order_for_print, pero TicketPreviewModal —el preview
-- que se abre desde la tarjeta de /orders— se compone a partir de OrderFeedItem,
-- que viene de esta funcion. Sin la clave aqui, pantalla y papel dirian cosas
-- distintas del mismo pedido: "Glovo · 954" en la una y "Glovo · 101755551192"
-- en el otro.
--
-- Dos lineas de cambio, las mismas que en order_for_print:
--   CTE `vivos`    -> s.platform_order_ref
--   CTE `tickets`  -> v.platform_order_ref
--
-- POR QUE VA LA DEFINICION ENTERA: tampoco estaba versionada. La ultima
-- completa del repo (20260725T1210_orders_feed_hitos.sql) tiene 8.943
-- caracteres de cuerpo frente a los 9.131 vivos. Reaplicar la del repo con el
-- cambio encima habria retrocedido 188 caracteres en silencio.
--
-- Transcrita de pg_get_functiondef y verificada byte a byte ANTES de tocarla:
-- md5 c67e06c9e4c904f21515f78727dcd58b (9.131 caracteres). Las dos lineas se
-- aplicaron por sustitucion sobre ese texto. Tras aplicar, el cuerpo vivo da
-- 8.987 caracteres y md5 65b1733292456e718fb241a374fa97bf, identico al
-- calculado en local: cero deriva en el transporte.
--
-- Finales de linea normalizados a LF a proposito (el catalogo los traia CRLF);
-- comprobado antes que ningun literal de cadena cruza un salto de linea, asi
-- que el cambio es de forma y no de dato.
--
-- ⚠️ Despues de 20260827214035: sin la columna, la funcion no resuelve.
--
-- NOTA: esta funcion no se puede invocar desde una conexion de servicio —
-- belongs_to_account() exige sesion autenticada—, asi que se verifico sobre
-- orders_feed_by_token (misma forma de CTE, 17 pedidos, clave presente) y
-- ejecutando las dos referencias nuevas contra `sale` por separado.
-- ============================================================================
