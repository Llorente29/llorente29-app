-- supabase/migrations/20260623T1700_worker_receipt_rls.sql
-- Aplicada: 2026-06-23
--
-- RECEPCIÓN POR EL TRABAJADOR (permisos). El trabajador (role=worker) entra con
-- sesión real (magic link) pero NO es manager; las tablas de recepción exigían
-- admin/manager → "Guardar" se caía en silencio. El trabajador SÍ debe poder
-- registrar una recepción (es su trabajo); NO debe tocar datos maestros.
--
-- (1) goods_receipt / goods_receipt_line: INSERT + SELECT para cualquier usuario
--     de la cuenta (belongs_to_account); UPDATE/DELETE siguen siendo manager
--     (corregir/anular es acción de oficina). El trabajador crea la recepción y
--     la deja en borrador; la confirmación va por RPC (abajo).
-- (2) confirm_goods_receipt: el guard pasa de admin/manager a belongs_to_account,
--     para que en modo "trabajador" un worker pueda confirmar (la decisión la
--     toma el toggle por local; en modo "oficina" el worker ni ve el botón).
-- (3) supplier / recipe_item_purchase_format: NO se tocan → el trabajador no crea
--     proveedores ni formatos. La creación de formato en el form es best-effort
--     (try/catch); lo que quede sin formato lo resuelve la oficina al confirmar.

-- ── (1) goods_receipt: separar por comando ──
...
CREATE POLICY goods_receipt_update ON public.goods_receipt
  FOR UPDATE USING (current_user_is_admin() OR current_user_is_admin_or_manager_of(account_id))
            WITH CHECK (current_user_is_admin() OR current_user_is_admin_or_manager_of(account_id));
CREATE POLICY goods_receipt_delete ON public.goods_receipt
  FOR DELETE USING (current_user_is_admin() OR current_user_is_admin_or_manager_of(account_id));

-- ── goods_receipt_line: separar por comando ──
DROP POLICY IF EXISTS goods_receipt_line_rw ON public.goods_receipt_line;
CREATE POLICY goods_receipt_line_select ON public.goods_receipt_line
  FOR SELECT USING (belongs_to_account(account_id));
CREATE POLICY goods_receipt_line_insert ON public.goods_receipt_line
  FOR INSERT WITH CHECK (belongs_to_account(account_id));
CREATE POLICY goods_receipt_line_update ON public.goods_receipt_line
  FOR UPDATE USING (current_user_is_admin() OR current_user_is_admin_or_manager_of(account_id))
            WITH CHECK (current_user_is_admin() OR current_user_is_admin_or_manager_of(account_id));
CREATE POLICY goods_receipt_line_delete ON public.goods_receipt_line
  FOR DELETE USING (current_user_is_admin() OR current_user_is_admin_or_manager_of(account_id));

-- ── (2) confirm_goods_receipt con guard belongs_to_account (resto idéntico a la
--     versión vigente: enrutado a zona + encolado CTB) ──
