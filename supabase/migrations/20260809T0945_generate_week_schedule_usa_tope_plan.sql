...
-- sobrescribir; producción va por delante del repo). Por eso esta migración es
-- idempotente y segura aunque el cuerpo vivo haya cambiado por otro lado.
--
-- Verificación post-ejecución (09/08):
--   definición viva contiene max_daily_minutes_plan · no queda la expresión vieja
--   generate_week_schedule (Alcalá, 10/08, cocina) → 18 bloques · 0 huecos · 69,00 h
--   NOTA: el resultado NO cambia todavía respecto a antes de la migración. El tope
--   de 9,5 h no muerde hasta que el motor se ancle a las plantillas reales
--   (turno Corrido1 14:45–00:15). El cambio es correcto pero LATENTE.
...
begin
  ...
  -- Ya aplicada: nada que hacer (idempotencia).
  if position('max_daily_minutes_plan' in v_def) > 0 then
    return;
  end if;
