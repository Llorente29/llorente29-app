-- 20260708T2310_shop_brand_menu_by_slug_gift.sql
--
-- STOREFRONT · display del REGALO por marca en el MENÚ DE MARCA. REGLA DE ORO: solo añadir.
--
-- Cuerpo copiado VERBATIM de la definición VIVA (leída con pg_get_functiondef; el
-- repo estaba desactualizado respecto a la BD). Único cambio: en el jsonb_build_object
-- del return, junto a 'free_delivery' (_shop_account_free_delivery), se añade
--
--   'gift' → public._shop_brand_free_gift(v_account_id, p_brand_id)
--
-- que devuelve {name, min, value} del plato de regalo activo de ESTA marca ahora, o
-- null. No se toca ningún otro campo/join/filtro.
--
...
begin
  ...
  return jsonb_build_object(
    ...
    -- (+ storefront paso3) Plato de regalo de esta marca, para el banner del menú.
    'gift', public._shop_brand_free_gift(v_account_id, p_brand_id),
    'categories', v_cats
  );
end;
