-- Aplicada 2026-08-07 por MCP. BUG DE MOTOR cazado por Code en la verificación de F10 (chips "?" rotos).
--
-- BUG: propose_schedule elegía entre TODOS los empleados de la cuenta -> proponía gente de Carabanchel
-- (Keilymar, Mirlenys, Marlón) para turnos de Alcalá. Code lo mitigó en la pantalla; el arreglo correcto
-- va en el MOTOR o cualquier otro consumidor del RPC hereda el fallo.
--
-- PRIMER INTENTO (demasiado estricto): filtrar solo por employees.location_id. Dejaba fuera a Marlón, que
-- tiene location_id=Carabanchel pero HA FICHADO 28 veces en Alcalá (polivalente real). Verificado contra
-- clock_entries.location_id_at_clock: Marlón 28 Alcalá / 38 Carabanchel; Keilymar y Mirlenys 0 en Alcalá.
-- Y `assigned_locations` está VACÍO en los 6 empleados, así que no sirve como fuente de verdad.
--
-- SOLUCIÓN (misma filosofía que la disponibilidad inferida: usar evidencia, no campos que nadie rellena).
-- Pertenece al roster de un local quien:
--   (a) lo tiene como location_id, (b) lo tiene en assigned_locations, o
--   (c) HA FICHADO ahí >= 5 veces en 90 días (umbral para descartar visitas puntuales).
-- Y se ordena poniendo primero a los de casa; quien cubre desde otro local sale con motivo explícito.
--
-- Verificado tras el arreglo (semana 24/08 Alcalá): Natacha y Pamela (de Alcalá) + Marlón marcado
-- "Cubre desde otro local (ha fichado aquí antes)" en 6 de 7 turnos. Keilymar y Mirlenys excluidas.
--
-- HALLAZGO COLATERAL (no es bug): Alcalá solo tiene 3 empleados propios y uno de vacaciones esa semana.
-- Antes el bug lo tapaba rellenando con gente de otro local. Ahora se ve la realidad: ese local va justo
-- de plantilla. Eso es información de negocio, no un fallo del generador.
    ...
    loop
      ...
      motivo := case
        ...
        when not r_cand.es_su_local
          then 'Cubre desde otro local (ha fichado aquí antes) · '||round(r_cand.horas_ya,1)||' h esta semana'
