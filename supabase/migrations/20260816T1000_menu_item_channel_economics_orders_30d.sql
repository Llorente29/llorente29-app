-- Frente "canal triplicado en Economía/modal" (ver docs/folvy_mapa_sistema.md).
--
-- menu_item_channel_economics ya devolvía una fila por canal×service_type cuando
-- channel_rate/brand_channel_rate tienen varias tarifas para el mismo canal (diseño
-- normal, no un bug de datos: Glovo/Uber/JustEat en Foodint tienen 3 tarifas reales
-- cada uno -- own_delivery/platform_delivery/pickup). Decisión de Julio (16/08): no
-- se elige un service_type canónico, se agrupan y etiquetan en la UI, ordenadas por
-- uso real. Este cambio SOLO añade el dato que la UI necesita para eso: cuántos
-- pedidos tuvo cada (canal, service_type) en los últimos 30 días de la cuenta.
--
-- Cambio de forma de retorno (columna nueva) -> CREATE OR REPLACE no vale para esto
-- (Postgres no permite cambiar el tipo de retorno de una función existente, error
-- 42P13), hace falta DROP + CREATE explícito. Probado antes contra datos reales con
-- una función _tmp_ (creada y borrada en la misma sesión de verificación): 0 filas de
-- diferencia frente a la función vieja en 10 productos reales de Foodint + con
-- p_location_id + con p_overrides, excluyendo la columna nueva -- estrictamente
-- aditivo, ningún otro cálculo cambia.

DROP FUNCTION IF EXISTS public.menu_item_channel_economics(uuid, jsonb, uuid);

CREATE FUNCTION public.menu_item_channel_economics(p_menu_item_id uuid, p_overrides jsonb DEFAULT NULL::jsonb, p_location_id uuid DEFAULT NULL::uuid)
...
BEGIN
  ...
  -- Uso real de los últimos 30 días de la cuenta, por (canal, service_type) --
  -- lo que la UI usa para decidir el modo dominante y atenuar el que no se usa.
  usage_30d AS (
...
COMMENT ON FUNCTION public.menu_item_channel_economics(uuid, jsonb, uuid) IS
  'Economía por canal de un producto. Una fila por (canal, service_type) -- un canal '
  'con varias tarifas configuradas (own_delivery/platform_delivery/pickup) devuelve '
  'varias filas a propósito, ninguna se descarta ni se elige como "la" canónica. '
  'orders_30d = pedidos reales de ese (canal, service_type) en los últimos 30 días de '
  'la cuenta -- lo usa la UI para ordenar el modo dominante primero y atenuar el que '
  'no se usa. p_location_id activa la cascada de effective_price() para ESE local.';
