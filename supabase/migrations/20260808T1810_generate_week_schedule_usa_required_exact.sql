...
-- Conecta generate_week_schedule a required_exact (sin redondear) en vez de required.
-- Las capas pasan a umbral (nivel - 0.5) = redondeo al entero mas proximo UNA sola vez,
-- en la descomposicion, no en cada celda.
--
-- NOTA DE HONESTIDAD: el "+101% de sobredimensionado" que consta en T1800 y en el commit
-- 95967a5 era un ERROR DE DIAGNOSTICO. Comparaba 72 personas-hora del motor con 39,4 de
-- "produccion pura", y no son la misma magnitud: las 72 son el coste de tener el local
-- abierto 10 h/dia con min_on_open=1, que es legitimo. El ceil aportaba <1 h. Este cambio
-- es por correccion, no porque arregle un inflado del doble.
...
begin
  ...
  -- required_exact (numeric), NO required (int)
  ...
  for r_b in
    ...
    niv as (select d.f,d.h,n.nivel from dem d cross join generate_series(1,6) n(nivel)
            where d.q >= n.nivel - 0.5)          -- redondeo al mas proximo, UNA vez
    ,
