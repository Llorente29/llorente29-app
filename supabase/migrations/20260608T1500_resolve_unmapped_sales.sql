-- 20260608T1500_resolve_unmapped_sales.sql
-- Aplicada: 2026-06-08
--
-- Acciones de resolución de la pantalla de excepciones del casado (Entrega B).
-- UNA RPC, tres acciones, sobre un producto (por nombre normalizado) de una cuenta:
--
--   action='link'   → SOLO para no_menu_item: el producto ya tiene receta resoluble
--                     (catalogProductId → organization_product_id → lastapp_product_map
--                      → recipe_item_id) y marca conocida; falta el menu_item. Lo crea
--                      (brand+recipe_item, precio del catálogo) y RECASA → las líneas casan.
--   action='ignore' → marca unmapped_reason='ignored' (excluido a propósito del análisis).
--   action='delist' → marca unmapped_reason='delisted' (producto retirado).
--
-- Reusa la MISMA cadena de resolución que recast_lastapp_sales (no la duplica): para
-- 'link' resuelve brand_id + recipe_item_id desde el JSON de las ventas, crea el
-- menu_item y delega el casado en recast_lastapp_sales (idempotente, respeta humano).
--
-- NO resuelve combos (lastapp product_type 'combo'): su coste es Σ de sus componentes,
-- no una receta plana. Si el producto es combo, 'link' lo rechaza con mensaje claro
-- (frente propio: coste de combo). 'ignore'/'delist' sí valen para combos.
--
-- SECURITY DEFINER + guard de tenancy. NO probar dentro de esta transacción
...
BEGIN
  ...
  IF p_action IN ('ignore','delist') THEN
    ...
    RETURN QUERY SELECT
      ...
      NULL::uuid, NULL::uuid, NULL::uuid, v_afect;
    RETURN;
  END IF;
