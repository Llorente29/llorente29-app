-- ============================================================
-- Rescatada del historial vivo de Supabase (aplicada 08/08/2026 17:22,
-- nunca versionada hasta ahora). Migración 9/12 de la secuencia F10
-- T1900→T2010 que rediseñó el reparto de horas de generate_week_schedule
-- en caliente sobre producción en una sola tarde.
-- ESTADO: por petición explícita de Julio ("el sistema puede tomarse el
-- tiempo que necesite: todas las pasadas necesarias para garantizar el
-- mejor reparto y el más justo para todos"), convierte cada fase de la
-- pasada 3 (3a refuerzo de pico, 3b apertura 17-20 entre semana, 3c
-- prep/limpieza 10-13, esta última nueva en esta migración) en un bucle
-- que repite hasta agotar posibilidades; assign_extra_block ya prioriza a
-- quien está más lejos de su jornada contratada, así la repetición
-- equilibra horas entre personas. Esta pasada 3 fue retirada por completo
-- en T2010 al detectarse que rellenaba horas sin comprobar demanda real.
-- Versión buena de la secuencia: T2010 (quitar_pasada3_relleno).
-- Resumen completo: memoria project_f10_solver_legal.
-- ============================================================

-- PASADA 3 CON REPETICION (criterio de Julio, 08/08):
-- "El sistema puede tomarse el tiempo que necesite: todas las pasadas necesarias
--  para garantizar el mejor reparto y el mas justo para todos."
-- Cada fase se repite hasta que no consigue colocar nada mas:
--   3a refuerzo de pico (dias fuertes primero)
--   3b apertura 17-20 entre semana
--   3c prep/limpieza (bloque declarado 10-13, previo a apertura)
-- Justicia: assign_extra_block elige SIEMPRE a quien mas lejos esta de su jornada
-- contratada, asi que la repeticion equilibra horas entre personas.
...
begin
  ...
  -- ===== PASADA 3a: DOBLAR EN EL PICO (repite hasta no poder mas) =====
  v_iter := 0;
  loop
    ...
    for r_p in
      ...
      pico as (select d.f,d.h,d.q, row_number() over (partition by d.f order by d.q desc, d.h) rn
               from dem d)
      select p.f, min(p.h) hi, max(p.h)+1 hf
      from pico p join carga c on c.f=p.f
      where p.rn <= 3 group by p.f, c.tot
      having max(p.h)+1 - min(p.h) <= 4
      order by max(c.tot) desc                    -- dias fuertes primero
    loop
  ...
  -- ===== PASADA 3b: ABRIR 17-20 ENTRE SEMANA (repite) =====
  v_iter := 0;
  loop
    v_iter := v_iter + 1; v_alguno := false;
    exit when v_iter > 30;
  ...
  -- ===== PASADA 3c: PREP / LIMPIEZA 10-13 (repite, ultimo recurso) =====
  v_iter := 0;
  loop
    v_iter := v_iter + 1; v_alguno := false;
    exit when v_iter > 30;
