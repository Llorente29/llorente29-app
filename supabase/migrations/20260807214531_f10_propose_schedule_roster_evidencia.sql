-- F10 (corrección 2) · El filtro por location_id era correcto pero DEMASIADO ESTRICTO: dejaba fuera a
-- Marlón, que tiene location_id=Carabanchel pero ha fichado 28 veces en Alcalá (es polivalente real).
-- `assigned_locations` está VACÍO en todos los empleados, así que no sirve como fuente de verdad.
--
-- Solución coherente con la filosofía de F10: usar la EVIDENCIA de los fichajes, no un campo que nadie
-- rellena. Pertenece al roster de un local quien: (a) lo tiene como location_id, (b) lo tiene en
-- assigned_locations, o (c) HA FICHADO ahí de verdad en los últimos 90 días (umbral >= 5 para descartar
-- visitas puntuales).
    ...
    for r_cand in
      ...
      where e.account_id = p_account and e.active
        and (
          ...
          -- evidencia real: ha fichado en este local (>=5 veces en 90 días)
      ...
      order by coalesce(av.available, true) desc,
               (e.location_id = p_location) desc,   -- primero los de casa, luego los que cubren
