...
-- ══════════════════════════════════════════════════════════════════════════
--
-- ✅ APLICADA el 09/09/2026 por MCP. La base la registró como 20260909081014 y
-- el fichero lleva ya ese nombre (regla 17). Va ANTES de desplegar el
-- importador que la usa: si se despliega el código primero, el importador
-- aborta diciendo que falta esta migración.
--
-- ── POR QUÉ ───────────────────────────────────────────────────────────────
--
...
--
-- ── PERMISOS: LO QUE ESTE FICHERO NO CONSIGUIÓ ────────────────────────────
--
-- ⚠️ Este fichero DECLARABA «EXECUTE revocado de PUBLIC: sólo service_role».
-- Era falso, y se comprobó justo después de aplicarlo:
--
--   service_role=EXECUTE, authenticated=EXECUTE, anon=EXECUTE, postgres=EXECUTE
--
-- `REVOKE ALL ... FROM public` quita el pseudo-rol PUBLIC, pero NO los GRANT
-- EXPLÍCITOS que este proyecto da a `anon` y `authenticated` sobre toda función
-- nueva de `public` (ALTER DEFAULT PRIVILEGES). No es un caso raro: de las 1.446
-- funciones de `public`, 1.169 tienen EXECUTE para `anon` y 1.331 para
-- `authenticated`. Es el valor por defecto de la casa.
--
-- **En este proyecto, un REVOKE FROM PUBLIC no deja una función privada. Hay
-- que revocar de `anon` y `authenticated` POR NOMBRE.**
--
-- Lo arregla la migración siguiente, 20260909081105, y no se toca aquí a
-- propósito: este fichero es el registro de lo que se ejecutó, con su error
-- dentro. Corregirlo en el sitio dejaría el repo diciendo que salió bien.
--
-- La lección es la del día: un comentario que AFIRMA un estado sin haberlo
-- medido es una suposición con aspecto de hecho. El REVOKE se escribió, se dio
-- por bueno, y nadie miró el ACL — hasta que lo miró Julio.
--
-- ── REVERSIBLE ────────────────────────────────────────────────────────────
--
...
-- lo que `pg_constraint` no enseñaba. Verificado al aplicar: las seis, y con
-- las columnas que este fichero anuncia.
--
-- Y el ACL, que es lo que este fichero se dejó:
--
...
--   -- tras 20260909081014:  {postgres=X/…, anon=X/…, authenticated=X/…, service_role=X/…}
--   -- tras 20260909081105:  {postgres=X/postgres, service_role=X/postgres}
--
