create or replace function public._set_product_availability_core(
  ...
  p_available_until timestamptz, p_reason_code text, p_account_id uuid,
...
create or replace function public.set_product_availability(
  p_menu_item_id uuid, p_is_available boolean, p_location_id uuid default null,
  p_reason text default 'manual', p_available_until timestamptz default null,
  p_reason_code text default null
) returns jsonb language plpgsql security definer set search_path to 'public' as $function$
declare
  v_account_id uuid; v_user uuid := auth.uid(); v_core jsonb; v_matriculas text[];
  v_affected_ids uuid[]; v_brands int; v_ext_locs text[]; v_channels int; v_secret text;
begin
    ...
    if v_secret is not null then
      perform net.http_post(
        ...
        body := jsonb_build_object(
          ...
          'location_id', p_location_id, 'available_until', p_available_until,
  ...
  return jsonb_build_object('brands', v_brands, 'channels', coalesce(v_channels, 0),
    'matriculas', coalesce(array_length(v_matriculas, 1), 0), 'location_id', p_location_id,
...
create or replace function public.set_product_availability_by_token(
  p_device_token text, p_menu_item_id uuid, p_is_available boolean,
  p_reason text default 'manual', p_available_until timestamptz default null,
  p_reason_code text default null
) returns jsonb language plpgsql security definer set search_path to 'public' as $function$
declare
...
create or replace function public.set_products_availability_bulk(
  p_menu_item_ids uuid[], p_is_available boolean, p_location_id uuid default null,
  p_reason text default 'manual', p_available_until timestamptz default null,
  p_reason_code text default null
) returns jsonb language plpgsql security definer set search_path to 'public' as $function$
declare
  v_user uuid := auth.uid(); v_account_id uuid; v_n_found int; v_n_mismatch int;
...
begin
  if p_menu_item_ids is null or array_length(p_menu_item_ids, 1) is null then
    raise exception 'set_products_availability_bulk: la seleccion esta vacia';
  end if;
  if array_length(p_menu_item_ids, 1) > 50 then
    raise exception 'set_products_availability_bulk: maximo 50 productos por operacion (recibidos %)', array_length(p_menu_item_ids, 1);
  end if;
  if p_reason is null or p_reason not in ('manual','stock_out','schedule') then
    raise exception 'set_products_availability_bulk: reason no valido %', p_reason;
  end if;
  if p_reason_code is not null and p_reason_code not in
     ('sin_stock','incidencia','fin_servicio','promocion','mantenimiento','otro') then
    raise exception 'set_products_availability_bulk: reason_code no valido %', p_reason_code;
  end if;
  ...
  select count(*), count(*) filter (where account_id <> v_account_id) into v_n_found, v_n_mismatch
  from menu_item where id = any(p_menu_item_ids);
  if v_n_found <> array_length(p_menu_item_ids, 1) then
    raise exception 'set_products_availability_bulk: algun producto de la seleccion no existe';
  end if;
...
create or replace function public.set_products_availability_bulk_by_token(
  p_device_token text, p_menu_item_ids uuid[], p_is_available boolean,
  p_reason text default 'manual', p_available_until timestamptz default null,
  p_reason_code text default null
) returns jsonb language plpgsql security definer set search_path to 'public' as $function$
declare
...
returns jsonb language plpgsql security definer set search_path to 'public' as $function$
declare
  v_device kds_device; v_term text := btrim(coalesce(p_query, ''));
begin
  v_device := public.kds_resolve_device(p_device_token);
  if v_device.id is null then raise exception 'search_products_by_token: token no valido'; end if;
  ...
  return coalesce((
    with rows as (
      ...
      where mi.account_id = v_device.account_id and b.is_active
        and mi.name ilike '%' || v_term || '%'
        and not exists ( select 1 from product_availability pa
      ...
      select pkey, (array_agg(id order by id))[1] as menu_item_id,
      ...
      from rows group by pkey
    )
    select jsonb_agg(jsonb_build_object('menuItemId', menu_item_id, 'name', name,
             'externalId', external_id, 'recipeItemId', recipe_item_id,
             'brands', brands, 'isCombo', is_combo) order by name)
    from grouped), '[]'::jsonb);
end;
$function$;

create or replace function public.search_products_86(
  p_account_id uuid, p_query text, p_location_id uuid default null
) returns jsonb language plpgsql security definer set search_path to 'public' as $function$
declare v_term text := btrim(coalesce(p_query, ''));
begin
  if not (public.current_user_is_admin() or public.current_user_is_admin_or_manager_of(p_account_id)) then
  ...
  return coalesce((
    with rows as (
      select mi.id, mi.external_id, mi.recipe_item_id, mi.brand_id, mi.name, mi.product_type,
             coalesce(mi.recipe_item_id::text, mi.external_id, mi.id::text) as pkey
      from menu_item mi join brand b on b.id = mi.brand_id
      where mi.account_id = p_account_id and b.is_active
        and mi.name ilike '%' || v_term || '%'
        and not exists ( select 1 from product_availability pa
...
create or replace function public._scope_preview_core(
  p_account_id uuid, p_matriculas text[], p_brand_ids uuid[], p_location_id uuid
) returns jsonb language plpgsql security definer set search_path to 'public' as $function$
...
returns jsonb language plpgsql security definer set search_path to 'public' as $function$
declare
  v_device kds_device; v_recipe_item uuid; v_external_id text; v_stock_group uuid;
  v_matriculas text[]; v_brand_ids uuid[]; v_brands int; v_core jsonb;
begin
  v_device := public.kds_resolve_device(p_device_token);
  if v_device.id is null then raise exception 'preview_scope_by_token: token no valido'; end if;

  select mi.recipe_item_id, mi.external_id, mi.stock_group_id
    into v_recipe_item, v_external_id, v_stock_group
  from menu_item mi where mi.id = p_menu_item_id and mi.account_id = v_device.account_id;
  ...
  select array_agg(distinct external_id) filter (where external_id is not null),
    ...
    into v_matriculas, v_brand_ids, v_brands from sib;

  v_core := public._scope_preview_core(v_device.account_id, v_matriculas, v_brand_ids, v_device.location_id);

  return jsonb_build_object('brands', coalesce(v_brands, 0),
...
returns jsonb language plpgsql security definer set search_path to 'public' as $function$
declare
  v_device kds_device; v_matriculas text[]; v_brand_ids uuid[]; v_brands int; v_core jsonb;
begin
  v_device := public.kds_resolve_device(p_device_token);
  if v_device.id is null then raise exception 'preview_scope_bulk_by_token: token no valido'; end if;
  if p_menu_item_ids is null or array_length(p_menu_item_ids, 1) is null then
    raise exception 'preview_scope_bulk_by_token: la seleccion esta vacia';
  end if;

  with sib as (
    select mi.id, mi.brand_id, mi.external_id from menu_item mi
    where mi.account_id = v_device.account_id
      and ( mi.id = any(p_menu_item_ids)
        or mi.recipe_item_id in (select recipe_item_id from menu_item
             where id = any(p_menu_item_ids) and account_id = v_device.account_id and recipe_item_id is not null)
        or mi.stock_group_id in (select stock_group_id from menu_item
