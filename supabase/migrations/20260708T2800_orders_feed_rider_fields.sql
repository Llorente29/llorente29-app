-- 20260708T2800_orders_feed_rider_fields.sql
--
-- COCINA · exponer los datos del RIDER de Catcher en el feed de pedidos.
-- REGLA DE ORO: solo añadir. Los cuerpos se copian VERBATIM de la definición VIVA
-- (leída con pg_get_functiondef; el repo estaba desactualizado). No se quita ni
-- cambia ningún campo/join/filtro existente.
--
-- Qué se añade (las columnas YA existen en sale, pobladas por catcher-webhook):
--   rider_transport_type, rider_seen_at, has_courier, rider_lat, rider_lng
--
-- Dos funciones:
--   1) orders_feed(uuid): ya traía el bloque de reparto (rider_name/phone/estado…);
--      se le añaden los 5 campos nuevos.
--   2) orders_feed_by_token(text): NO traía NINGÚN campo de reparto (drift). Se le
--      añade el bloque de reparto COMPLETO + los 5 campos nuevos, para paridad con
--      orders_feed (la Estación de Tablet mostrará el rider igual que la sesión).
--
-- El JSON se arma con to_jsonb(t) sobre el CTE `tickets`; por eso los campos deben
-- estar tanto en `vivos` (origen) como en `tickets` (lo que se serializa).
--
-- CREATE OR REPLACE (DDL puro) → seguro en SQL Editor pese a SECURITY DEFINER; los
-- GRANT se conservan al reemplazar.
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2) orders_feed_by_token(text) — se le añade el bloque de reparto COMPLETO
--    (antes no traía ninguno) + los campos de rider de Catcher.
-- ─────────────────────────────────────────────────────────────────────────────
