-- ----------------------------------------------------------------------------
-- Folvy - 20260815T0400
-- Ficha por producto (Tramo C): reconstruir fichas desde el histórico
-- ----------------------------------------------------------------------------
--
-- QUE ES ESTO
-- -----------
-- Con el código ya rellenado (Tramo B), por cada (supplier_id, supplier_code)
-- con líneas reales: si no había ficha, se crea SOLO cuando hay evidencia
-- suficiente (el intérprete de Ley 1.bis confirma la mediana histórica, o la
-- mediana tiene >=3 usos reales sin señal de intérprete disponible -- criterio
-- propio, declarado, mismo umbral que ya usa todo el proyecto para
-- "histórico fuerte"). Si existe y coincide, no se toca. Si existe y
-- contradice al histórico fuerte, o apunta a un artículo distinto del que
-- usan sus propias líneas, NO se autocorrige -- va al informe
-- (docs/folvy_ficha_por_producto_reconstruccion_20260815.md).
--
-- RESULTADO MEDIDO
-- ----------------
-- 182 códigos con histórico real evaluados: 102 ya coincidían, 17 fichas
-- nuevas creadas aquí (source='albaran', sin verificar), 35 sin evidencia
-- suficiente para crear solas, 21 contradicen la ficha existente, 10
-- apuntan a un artículo distinto de sus propias líneas, 3 candidatas donde
-- el intérprete contradice al histórico (no creadas). Informe completo con
-- las cuatro tablas en docs/folvy_ficha_por_producto_reconstruccion_20260815.md.
--
-- Nada de esto toca stock: movimientos source_type='goods_receipt_line' de
-- Llorente29 siguen en 658 antes y después.
-- ----------------------------------------------------------------------------
...
declare
  ...
  v_format_id uuid;
begin
  -- Universo: (supplier_id, supplier_code) con lineas reales, articulo
  -- dominante y mediana de qty_in_base/qty_received.
  ...
  where fa.recipe_item_id is null; -- solo "no_existe"

  -- Cruce con el interprete: representante de raw_text+pack_size por codigo.
