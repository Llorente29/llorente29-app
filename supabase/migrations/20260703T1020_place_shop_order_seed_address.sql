-- 20260703T1020_place_shop_order_seed_address.sql
-- Aplicada: (pendiente)
--
-- F4·T1 — SIEMBRA SILENCIOSA de dirección. Rama ADITIVA al final de place_shop_order
-- (tras resolver v_customer y vincular la venta, antes del canje del cupón): si hay
-- customer, el modo es 'delivery' y el payload trae address, se upsertea en
-- customer_address por (customer_id, lower(address)). Si no existe, la crea (como
-- predeterminada si es la primera del cliente); si existe, refresca detail/lat/lng.
-- TODO el bloque va en begin/exception when others then null: JAMÁS puede tumbar una
-- venta (la venta ya está creada y vinculada en este punto).
--
-- Resto de la función IDÉNTICO a 20260702T2100_place_shop_order_welcome_needs_contact.sql.
-- Único cambio: declare v_seed_addr + bloque de siembra marcado "F4·T1".
