-- supabase/migrations/20260620T0100_orders_feed_platform_codes.sql
-- Aplicada: 2026-06-20 (SQL Editor) — versionada para evitar drift.
-- ============================================================================
-- Expone platform_order_code (nº real de plataforma) + pos_short_code (corto del
-- pedido) en orders_feed, para el renderizador de tickets. Único cambio vs
-- 20260619T1700: la CTE vivos selecciona los 2 campos y tickets los pasa al JSON.
-- Todo lo demás (marca, familias por línea, hijas con group_type, notas,
-- alérgenos, marcado, orden lógico) intacto. security definer + belongs_to_account.
-- Idempotente, sin BEGIN/COMMIT, sin SELECT de prueba.
-- ============================================================================
...
begin
  ...
  with vivos as (
    select s.id, s.external_ref, s.external_tab_ref,
           s.platform_order_code, s.pos_short_code,          -- NUEVO
  ...
  tickets as (
    select v.id as sale_id, v.external_ref, v.external_tab_ref,
           v.platform_order_code, v.pos_short_code,          -- NUEVO
