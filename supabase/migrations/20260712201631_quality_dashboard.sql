...
language sql stable security invoker as $$
  ...
  select jsonb_build_object(
    ...
    'by_brand', (select coalesce(jsonb_agg(x order by x.n desc),'[]'::jsonb) from (
      select brand, round(avg(stars),2) as avg, count(*) as n,
        count(*) filter (where stars<=2) as neg
...
comment on function public.quality_dashboard is 'Area de Calidad: valoraciones (dist/avg/por marca), comentarios, etiquetas, tipos de error, platos que fallan, reembolsos. Lee channel_review + channel_incident.';