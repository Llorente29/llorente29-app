...
-- SEGUNDA PASADA: las horas contratadas que sobran no se tiran.
-- Se acoplan alargando un turno ya asignado hacia la hora contigua con mas demanda
-- SIN CUBRIR. Mantiene el turno continuo y respeta jornada maxima y 12 h entre jornadas.
--
-- NOTA (superada por 20260808T1540 en el mismo dia): esta version usa CREATE TEMPORARY
-- TABLE dentro de una funcion STABLE, lo cual falla. Se versiona igualmente porque es lo
-- que se aplico en produccion en ese momento.
...
declare
  ...
  v_plan jsonb := '[]'::jsonb;      -- turnos asignados, mutable
  v_gaps jsonb := '[]'::jsonb;      -- bloques sin cubrir
  v_ini_ts timestamp; v_fin_ts timestamp; v_dur numeric;
  v_found boolean; v_target numeric; v_i int; v_pass int; v_cambio boolean;
begin
  ...
  create temporary table if not exists _dem(f date, h int, q int) on commit drop;
  delete from _dem;
  insert into _dem
  select r.fecha, r.hora, r.required
  ...
  -- PASADA 1: cubrir las capas de demanda
  for r_b in
    with win as (select f, min(h) w_ini, max(h)+1 w_fin from _dem group by f),
    niv as (select d.f, d.h, n.nivel from _dem d cross join generate_series(1,6) n(nivel)
            where d.q>=n.nivel),
    ...
    part as (select e.f,e.nivel,e.hi2,e.hf2,
                    ceil((e.hf2-e.hi2)::numeric / v_max_h)::int n_p
    ...
    select f, extract(isodow from f)::int-1 d, nivel, hi3, hf3, (hf3-hi3)::numeric horas
  ...
  -- PASADA 2: acoplar las horas sobrantes donde queda demanda sin cubrir
  v_pass := 0;
  loop
    ...
    for r_x in
      with plan as (
        select (x->>'f')::date f, (x->>'ini')::int ini, (x->>'fin')::int fin,
               (x->>'emp')::uuid emp, ord.i - 1 AS idx
        from jsonb_array_elements(v_plan) with ordinality ord(x,i)
      ),
      cob as (  -- cuanta gente cubre ya cada hora
      ...
      select p.idx, p.f, p.ini, p.fin, p.emp, d.q - coalesce(c.n,0) AS deficit_hora
      from plan p
      join deficit df on df.emp = p.emp and df.falta >= 1
      join _dem d on d.f = p.f and d.h = p.fin           -- hora justo despues del turno
      left join cob c on c.f = p.f and c.hora = p.fin
      where d.q - coalesce(c.n,0) > 0                    -- esa hora aun necesita gente
        and (p.fin + 1 - p.ini) <= v_max_h               -- no supera jornada maxima
        and (v_fin->>p.emp::text) is null
      order by (d.q - coalesce(c.n,0)) desc, p.f, p.ini
      limit 1
    loop
      v_plan := jsonb_set(v_plan, array[r_x.idx::text,'fin'], to_jsonb(r_x.fin+1));
      v_plan := jsonb_set(v_plan, array[r_x.idx::text,'ext'],
                  to_jsonb(coalesce((v_plan->r_x.idx->>'ext')::int,0)+1));
