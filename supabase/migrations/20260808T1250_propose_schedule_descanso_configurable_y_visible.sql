...
-- 1) El solver deja de asumir 36 h: lee el umbral de break_policy (multiempresa).
-- 2) Nueva funcion que EXPONE el descanso semanal de cada persona para que la UI lo pinte.
--
-- NOTA: has_weekly_rest_36h (de 20260808T1220) queda HUERFANA a partir de aqui,
-- sustituida por has_weekly_rest (umbral configurable via weekly_rest_minutes_for).
-- No se borra sin visto bueno de Julio (regla de no destruccion) — queda declarada
-- en el repo, sin uso, hasta que se decida.
...
declare
  ...
  v_rest_min int;      -- descanso semanal exigido (minutos), por cuenta/local
  v_rest_12h int;      -- descanso entre jornadas (minutos), por cuenta/local
begin
...
-- QUE SE VEA: descanso semanal de cada persona en la propuesta
CREATE OR REPLACE FUNCTION public.propose_schedule_rest(
...
RETURNS TABLE(
  ...
  minimo_exigido_horas numeric, cumple boolean)
LANGUAGE sql STABLE
SET search_path TO 'public','pg_temp'
AS $$
  with req as (select public.weekly_rest_minutes_for(p_account,p_location) as m),
  t as (
  ...
  select b.employee_id, b.employee_name,
         ...
         round((select m from req)/60.0,1),
         b.h >= (select m from req)/60.0
  from best b order by b.employee_name;
$$;

DO $guard$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname='propose_schedule_rest') THEN
    RAISE EXCEPTION 'propose_schedule_rest no quedo';
  END IF;
END $guard$;
