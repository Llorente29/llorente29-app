-- ============================================================
-- Rescatada del historial vivo de Supabase (aplicada 08/08/2026 16:59,
-- nunca versionada hasta ahora). Migración 2/12 de la secuencia F10
-- T1900→T2010 que rediseñó el reparto de horas de generate_week_schedule
-- en caliente sobre producción en una sola tarde.
-- ESTADO: revierte T1900 (orden por demanda vaciaba lunes-jueves) — vuelve
-- al orden cronológico. Punto de partida real para el rediseño de T1920.
-- Versión buena de la secuencia: T2010 (quitar_pasada3_relleno).
-- Resumen completo: memoria project_f10_solver_legal.
-- ============================================================

-- REVERT de T1900. Ordenar por demanda descendente empeoro el resultado:
-- el motor agotaba las horas contratadas en viernes/sabado/domingo y dejaba
-- lunes-jueves COMPLETAMENTE vacios (0 turnos, 10 huecos).
-- Ademas el descanso de 36 h bloqueaba a la plantilla tras el fin de semana.
--
-- DIAGNOSTICO REAL: no es un problema de orden. Un reparto voraz gasta el presupuesto
-- de horas en lo primero que mira, ordene como ordene. La solucion correcta es repartir
-- las horas contratadas PROPORCIONALMENTE a la demanda de cada dia ANTES de asignar
-- (presupuesto por dia), y luego asignar dentro de cada dia. Eso es un rediseno del
-- reparto, no un ORDER BY. Queda declarado como pendiente.
--
-- Se vuelve al orden cronologico, que al menos cubre los 7 dias.
