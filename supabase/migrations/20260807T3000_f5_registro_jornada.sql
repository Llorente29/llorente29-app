-- Aplicada 2026-08-07 por MCP. Verificado sobre julio 2026 (Llorente29): los totales cuadran EXACTO con
-- compute_employee_balance de F2 (Natacha +23,68 / Johanny -1,67). Turno partido visible (Johanny: 24 días
-- en 35 tramos). Vacaciones del 13-14/07 constan como tales, no desaparecen del registro.
--
-- F5.1 · REGISTRO DE JORNADA (RD-ley 8/2019, art. 34.9 ET).
-- Cumple: identificación de empresa/trabajador (accounts.legal_name + cif, employees.dni — verificados
-- rellenos), registro DIARIO con hora de inicio y fin, totales, conservación 4 años.
-- Las horas son las REALES (real_datetime vía team_worked_shifts), NUNCA las redondeadas: el redondeo no
-- es oponible en inspección.
-- Devuelve TODOS los días del periodo, incluidos los no trabajados (en el registro deben constar, no
-- desaparecer). Una jornada PARTIDA aparece como dos tramos del mismo día: ambos deben constar.
...
language sql stable
as $function$
  with dias as (select generate_series(p_from, p_to, interval '1 day')::date as d),
  ...
  select dias.d, j.started_at, j.ended_at,
...
-- F5.1b · Totales para cabecera/pie del PDF y para el export de gestoría.
-- Una jornada partida suma como UN día trabajado (pero sus tramos constan en el detalle).
