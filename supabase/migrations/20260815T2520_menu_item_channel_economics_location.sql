-- Fase 3, B.2 (15/08/2026) -- menu_item_channel_economics gana p_location_id
-- uuid DEFAULT NULL, aditivo. Inventario previo (RECON): exactamente 2
-- llamadores en toda la app -- EconomiaTab.tsx (sin preview, sin location) y
-- EditPricesModal.tsx (con preview) -- ambos pasan por el único caller de la
-- RPC, getMenuItemChannelEconomics() en menuOverrideService.ts. Ninguno pasa
-- location_id hoy: con el parámetro omitido (NULL), el comportamiento es
-- BYTE A BYTE el de siempre (effective_price() con p_location_id=NULL salta
-- los tramos 1-2 de su propia cascada, igual que antes).
--
-- UNA CASCADA, UNA IMPLEMENTACIÓN (instrucción explícita de Julio): el precio
-- efectivo ya NO se calcula aquí con su propio COALESCE -- se delega en
-- effective_price(p_menu_item_id, channel_id, p_location_id), la misma
-- función que ya usan set/clear_menu_item_override. Esta función solo AÑADE
-- el EXISTS puntual que necesita para clasificar price_source/is_location_override
-- (saber POR QUÉ salió ese número, no volver a decidir CUÁL es).
--
-- Deuda declarada, NO resuelta aquí (fuera del pedido explícito de Julio):
-- is_available sigue leyendo solo el override de marca/canal (location_id IS
-- NULL) -- un 86 puesto específicamente en un local (menu_item_override con
-- location_id y channel_id/channel_id NULL) no se refleja todavía en esta
-- RPC cuando se pasa p_location_id. No bloquea el selector de precios (B.2
-- tal como está pedido), pero es un hueco real si algún día el 86 se pone
-- por local desde aquí.
--
-- CREATE OR REPLACE FUNCTION no sustituye una firma con distinta lista de
-- parámetros -- crea una SOBRECARGA (feedback_create_or_replace_function_overload).
-- Hay que DROPear la firma vieja de 2 parámetros explícitamente antes, o
-- "menu_item_channel_economics(uuid)" queda ambiguo entre las dos.
...
returns table(
  channel_id uuid, channel_name text, channel_type text, service_type text,
  price numeric, price_source text, is_location_override boolean, is_available boolean,
  vat_rate numeric, price_with_vat numeric, cost numeric, packaging_cost numeric,
  food_cost numeric, cost_available boolean, commission_pct numeric, commission_base text,
  commission_amount numeric, commission_fixed numeric, own_courier_cost numeric,
  own_customer_fee numeric, order_costs_per_item numeric, contribution_margin numeric,
  contribution_margin_pct numeric, net_margin numeric, net_margin_pct numeric,
  food_cost_pct numeric, target_food_cost_pct numeric, food_cost_status text,
  plate_cost_pct numeric, target_plate_cost_pct numeric, plate_cost_status text
)
language plpgsql
...
BEGIN
  ...
  ov AS (
    -- 86 de marca/canal (sin local) -- ver deuda declarada arriba sobre 86 por local.
  ...
  eff AS (
    ...
    LEFT JOIN LATERAL (
      -- Mismos tramos de effective_price() (1-2: local+canal/local solo;
      -- 3: canal de marca) -- solo para ETIQUETAR de dónde sale el número,
      -- el número ya lo decidió effective_price() arriba. No es una segunda
      -- cascada de precio, es la misma cascada preguntada por existencia.
      SELECT
        EXISTS (
