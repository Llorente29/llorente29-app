...
--
-- Se comprueba sobre el CÓDIGO, no sobre el texto completo de la función:
-- pg_get_functiondef() devuelve también los comentarios del cuerpo, y el
-- comentario del paso 1) de aquí arriba MENCIONA "compute_sale_line_consumption"
-- al explicar qué se quitó de close_sale. Un ILIKE ingenuo sobre el texto
-- entero dispara un FALSO POSITIVO con ese comentario y bloquea la migración
-- aunque ya no la llame nadie de verdad (así se aplicó por primera vez: hubo
-- que aplicar las 4 funciones sin esa frase para que el guard pasara).
-- regexp_replace quita cada línea "--…" antes de buscar, así que el guard solo
-- dispara si el nombre aparece en CÓDIGO real (una llamada), nunca en prosa.
-- ────────────────────────────────────────────────────────────────────────────
do $guard$
declare
  v_callers text;
begin
  ...
  where n.nspname = 'public'
    ...
    and regexp_replace(pg_get_functiondef(p.oid), '--[^\n]*', '', 'g')
        ilike '%compute_sale_line_consumption%';
